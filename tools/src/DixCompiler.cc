...
bool has_successor(xmlNode* node) {
  for (xmlNode* n = node->next; n != NULL; n = n->next) {
  ...
  for(xmlNode* elem = ent->children; elem != NULL; elem = elem->next) {
    ...
    if (nameIs(elem, "i")) {
      ...
      if (name == "") {
        ...
        if (current_par_name.empty() && is_first) {
          if (prefix_pars.find(name) == prefix_pars.end()) {
            to = t->add_state();
            t->insert_transducer(from, to, *pars[name]);
            prefix_pars[name] = to;
          }
          from = prefix_pars[name];
        } else if (current_par_name.empty() && !has_successor(elem)) {
          if (suffix_pars.find(name) == suffix_pars.end()) {
            HfstState a = t->add_state();
            HfstState b = t->add_state();
            t->insert_transducer(a, b, *pars[name]);
            suffix_pars[name] = std::make_pair(a, b);
          }
          to = suffix_pars[name].first;
          HfstBasicTransition e(to, "@_EPSILON_SYMBOL_@", "@_EPSILON_SYMBOL_@", weight);
          weight = 0;
          from = suffix_pars[name].second;
        } else {
          to = t->add_state();
          t->insert_transducer(from, to, *pars[name]);
          from = to;
