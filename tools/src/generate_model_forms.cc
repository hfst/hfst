...
std::ostream &
operator<<(std::ostream &out, const StringVector &v)
{
...
HfstTokenizer
get_alphabet_string_tokenizer(HfstTransducer &fst)
{
...
StringVector
get_analysis_prefix(const StringVector &reversed_analysis)
{
    StringVector prefix;
...
StringVector
join(StringVector sv1, const StringVector &sv2)
{
...
bool
contains_analysis_symbols(const StringVector &word_form)
{
    for (StringVector::const_iterator it = word_form.begin();
         it != word_form.end(); ++it)
    {
        const std::string &symbol = *it;
        if (symbol.size() > 1 and symbol[0] == '['
            and symbol[symbol.size() - 1] == ']')
        {
            return true;
        }
    }
...
StringVector
generate_word_forms(const StringVector &analysis,
                    HfstTransducer &form_generator, size_t max_generated_forms,
                    float generate_threshold)
{
         ...
         it != word_forms->end(); ++it)
    {
        ...
        result_set.insert(StringVector(word_form.rbegin(), word_form.rend()));
...
StringVectorVector
get_model_forms(const StringVector &reversed_analysis,
                ...
                float generate_threshold)
{
    ...
    for (StringVectorVector::const_iterator it = model_forms.begin();
         it != model_forms.end(); ++it)
    {
        StringVector model_analysis = join(*it, reversed_analysis_prefix);

        results.push_back(generate_word_forms(model_analysis, form_generator,
                                              max_generated_forms,
                                              generate_threshold));
    }
...
StringPair
split(const std::string &line, const std::string &separator)
{
...
StringVector
read_model_form(std::istream &in, HfstTokenizer &tokenizer)
{
    std::string line;
    std::getline(in, line);
...
StringVectorVector
read_model_forms(const std::string &model_form_filename,
                 HfstTokenizer &tokenizer)
{
...
StringVectorVector
get_paradigms(const std::string &word_form, const StringVectorVector &guesses,
              HfstTransducer &generator, const StringVectorVector &model_forms,
              size_t number_of_generated_forms, float generate_threshold)
{
         ...
         it != guesses.end(); ++it)
    {
        ...
        StringVectorVector results
            = get_model_forms(analysis_guess, model_forms, generator,
                              number_of_generated_forms, generate_threshold);
        ...
        paradigm.insert(paradigm.end(), rev_analysis_guess.begin(),
             ...
             jt != results.end(); ++jt)
        {
            ...
            paradigm.push_back("\t");
            paradigm.insert(paradigm.end(), model_form.begin(),
                            model_form.end());
        }
...
main(int argc, char *argv[])
{
    ...
    std::cerr << "Reading guesser and generator from file " << argv[1]
              << std::endl;
    ...
    while (std::cin.peek() != EOF)
    {
        ...
        StringVectorVector guesses
            = get_guesses(line, guesser, MAX_ANALYSES, tokenizer);

        StringVectorVector guess_model_forms = get_paradigms(
            line, guesses, generator, model_forms, MAX_GENERATED_FORMS);

        for (StringVectorVector::const_iterator it = guess_model_forms.begin();
             it != guess_model_forms.end(); ++it)
        {
