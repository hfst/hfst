...
NewLexicalModel::NewLexicalModel(const std::string &filename,
                                 std::istream *paradigm_guess_stream)
    : in(filename), lexical_model(in),
      upper_case_interpolator(8.79), // <----- FIX ME!!!
      lower_case_interpolator(31.0), // <----- FIX ME!!!
      id(0), lexical_model_is_broken(false),
...
void
NewLexicalModel::initialize_tag_probabilities(void)
{
    ...
    for (HfstOneLevelPaths::const_iterator it = upper_case_tag_paths->begin();
         it != upper_case_tag_paths->end(); ++it)
    {
    ...
    for (HfstOneLevelPaths::const_iterator it = lower_case_tag_paths->begin();
         it != lower_case_tag_paths->end(); ++it)
    {
        float penalty = it->first;
...
std::string
NewLexicalModel::to_lower_case(const std::string &word) const
{
    std::string ret = word;
...
const WeightedStringVector &
NewLexicalModel::operator[](const std::string &word)
{
...
StringVector
NewLexicalModel::split(const std::string &line)
{
    StringVector result;
    ...
    do
    {
        ...
        result.push_back(
            line.substr(old_tab_pos + 1, tab_pos == std::string::npos
                                             ? std::string::npos
                                             : tab_pos - old_tab_pos - 1));
        old_tab_pos = tab_pos;
    } while (old_tab_pos != std::string::npos
             && old_tab_pos != line.size() - 1);

    return result;
}

float
NewLexicalModel::get_weight(const std::string &weight_string)
{
...
const WeightedStringVector &
NewLexicalModel::paradigm_guess(const std::string &word,
...
NewLexicalModel::cache_word_analyses(const std::string &word)
{
    ...
    if (word.size() > 0)
    {
        ...
        else if (word.find("Å") == 0 or word.find("Ä") == 0
                 or word.find("Ö") == 0)
        {
            upper_case = true;
        }
...
float
NewLexicalModel::get_prob(float weight)
{
    return exp(-1 * weight);
}

float
NewLexicalModel::get_penalty(float prob)
{
    return -1 * log(prob);
}

std::string
NewLexicalModel::to_string(StringVector::const_iterator begin,
...
const WeightedStringVector &
NewLexicalModel::guess(const std::string &word, bool upper_case,
                       const StringVector &rev_tokenized_word)
{
    WeightedStringVector &word_analyses = analysis_cache[word];

    word_analyses = upper_case ? upper_case_empty_suffix_null
                               : lower_case_empty_suffix_null;

    size_t suffix_length
        = std::min(rev_tokenized_word.size(), static_cast<size_t>(100));
    ...
    for (size_t i = suffix_length; i > 0; --i)
    {
        const WeightedStringVector &suffix_analyses
            = get_suffix_analyses(word, rev_tokenized_word.begin(),
                                  rev_tokenized_word.begin() + i, upper_case);
    ...
    merge_analyses(upper_case ? upper_case_empty_suffix_guesses
                              : lower_case_empty_suffix_guesses,
                   word_analyses, upper_case);

    // For Finnish
    word_analyses = get_suffix_analyses(
        word, rev_tokenized_word.begin(),
        rev_tokenized_word.begin() + last_found_suffix_length, upper_case);
...
bool
NewLexicalModel::is_oov(const std::string &word)
{
...
size_t
NewLexicalModel::get_suffix_length(StringVector::const_iterator begin,
                                   StringVector::const_iterator end)
{
    size_t length = 0;
...
const WeightedStringVector &
NewLexicalModel::get_suffix_analyses(const std::string &word,
                                     StringVector::const_iterator begin,
                                     StringVector::const_iterator end,
                                     bool upper_case)
{
    AnalysisCache &suffix_cache
        = upper_case ? upper_case_suffix_cache : lower_case_suffix_cache;
    std::string suffix
        = word.substr(word.size() - get_suffix_length(begin, end));
...
const WeightedStringVector &
NewLexicalModel::cache_suffix_analyses(const std::string &word,
                                       ...
                                       StringVector::const_iterator end)
{
    ...
    if (not word.empty())
    {
        ...
        else if (word.size() > 1)
        {
            ...
            if (initial_segment == "Å" or initial_segment == "Ä"
                or initial_segment == "Ö")
            {
                upper_case = true;
            }
        }
...
void
NewLexicalModel::merge_analyses(const WeightedStringVector &suffix_analyses,
                                ...
                                bool upper_case)
{
         ...
         it != word_analyses.rend(); ++it)
    {
        ...
        // Check this.
        it->first
            = ((upper_case ? upper_case_interpolator : lower_case_interpolator)
                   * it->first
               + (id == id_weight_pair.first ? id_weight_pair.second : 0.0))
              / (1
                 + (upper_case ? upper_case_interpolator
                               : lower_case_interpolator));
    }
}

float
NewLexicalModel::get_suffix_penalty(const std::string &suffix, bool upper_case)
{
    ProbabilityMap::iterator it
        = (upper_case ? upper_case_suffix_penalties.find(suffix)
                      : lower_case_suffix_penalties.find(suffix));

    if (it
        == (upper_case ? upper_case_suffix_penalties.end()
                       : lower_case_suffix_penalties.end()))
    {
        HfstOneLevelPaths *suffix_penalty_paths = lexical_model.lookup(
            suffix + (upper_case ? "<upper_suffix>" : "<lower_suffix>"));
        ...
        delete suffix_penalty_paths;
        (upper_case ? upper_case_suffix_penalties[suffix]
                    : lower_case_suffix_penalties[suffix])
            = penalty;
        return penalty;
    }
...
float
NewLexicalModel::get_tag_penalty(const std::string &tag, bool upper_case)
{
    if (upper_case)
    {
        if (upper_case_tag_penalties.find(tag)
            == upper_case_tag_penalties.end())
        {
            throw InvalidKey();
        }
    }
    else if (lower_case_tag_penalties.find(tag)
             == lower_case_tag_penalties.end())
    {
        throw InvalidKey();
    }

    float penalty = (upper_case ? upper_case_tag_penalties[tag]
                                : lower_case_tag_penalties[tag]);
...
{
    bool
    operator()(const WeightedString &w1, const WeightedString &w2) const
    {
        // w1 should be earlier if it's more probable than w2 or
        // alphabetically precedes it.
        return w1.first > w2.first
               or (w1.first == w2.first and w1.second < w2.second);
    }
};

typedef std::priority_queue<WeightedString, std::vector<WeightedString>,
                            OrderWeightedStrings>
    WeightedStringQueue;

void
preserve_n_best_analyses(WeightedStringVector &word_analyses,
                         size_t number_of_guesses)
{
    WeightedStringQueue analysis_queue(word_analyses.begin(),
                                       word_analyses.end());
...
void
NewLexicalModel::bayesian_invert(WeightedStringVector &word_analyses,
                                 const std::string &suffix, bool upper_case)
{
    ...
    for (WeightedStringVector::iterator it = word_analyses.begin();
         it != word_analyses.end(); ++it)
    {
        it->first = suffix_penalty + get_penalty(it->first)
                    - get_tag_penalty(it->second, upper_case);
        it->first *= 0.2;
    }
}
