...
float
divide_by_sum_of_weights(float weight)
{
    if (sum_of_weights == 0)
        return 0;
...
float
take_negative_logarithm_e(float weight)
{
    float result;
    if (weight == 0)
...
float
take_negative_logarithm_10(float weight)
{
    float result;
    if (weight == 0)
        result = 0; // shoud be INFINITY, but doesn't work in transitions
...
print_usage()
{
    ...
    fprintf(
        ...
        "      --norm                Divide each weight by sum of all "
        "weights\n"
        "                            (with option -j)\n"
        "      --log                 Take negative natural logarithm of each "
        "weight\n"
        "      --log10               Take negative 10-based logarithm of each "
        "weight\n"
        "  -p, --pairstrings         Input is in pairstring format\n"
        "  -S, --has-spaces          Input has spaces between symbols/symbol "
        "pairs\n"
        "  -e, --epsilon=EPS         Interpret string EPS as epsilon.\n"
        "  -m, --multichar-symbols=FILE   Strings that must be tokenized as "
        "one symbol.\n");
    fprintf(message_out, "\n");

    fprintf(
        message_out,
        "If OUTFILE or INFILE is missing or -, standard streams will be "
        "used.\n"
...
process_stream(HfstOutputStream &outstream)
{
    ...
    hfst::HfstStrings2FstTokenizer multichar_symbol_tokenizer(
        multichar_symbols, std::string(epsilonname));
