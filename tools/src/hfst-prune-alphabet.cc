...
print_usage()
{
    ...
    fprintf(message_out, "Alphabet pruning options:\n"
...
process_stream(HfstInputStream& instream, HfstOutputStream& outstream)
{
  ...
  //outstream.open();
    ...
    while(instream.is_good())
    {
        ...
        if (transducer_n==1)
        {
          verbose_printf("Pruning %s...\n", inputname);
        }
        else
        {
          verbose_printf("Pruning %s... " SIZE_T_SPECIFIER "\n", inputname,
                         transducer_n);
        }
...
int main( int argc, char **argv ) {
  ...
  _setmode(1, _O_BINARY);
    ...
    hfst_set_program_name(argv[0], "0.1", "HfstPruneAlphabet");
