...
CapitalizationState
TokenIOStream::get_capitalization_state(
    const SymbolNumberVector &symbols) const
{
    if (symbols.size() == 0)
        return Unknown;

    const SymbolNumber &first = symbols[0],
                       last = symbols.size() > 1 ? symbols[1] : symbols[0];
...
TokenIOStream::get_capitalization_state(const TokenVector &tokens) const
{
    ...
    const Token &first = tokens[0],
                last = tokens.size() > 1 ? tokens[1] : tokens[0];
...
TokenIOStream::read_delimited(const char delim)
{
    ...
    if (is_wblank)
    {
        ...
        if (c != EOF)
        {
            ...
            {
                stream_error(
                    std::string("Error in parsing a wordbound blank"));
            }
            else
            {
                result += c;
...
TokenIOStream::read_token()
{
    ...
    if (escaped_chars.find(next_char) != escaped_chars.end())
    {
        ...
        case '<':
        {
            ...
            if (t.type == Symbol
                && alphabet.is_tag(
                    t.symbol)) // the '<' introduced a tag, that's fine
                return t;
