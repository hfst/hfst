...
#include "inc/getopt-cases-common.h"
        ...
        case 'x':
        {
            ...
            if (strcmp(argument, "yes") == 0 || strcmp(argument, "true") == 0
...
#include "inc/check-params-common.h"
    ...
    {
        lexcfilenames
            = static_cast<char **>(malloc(sizeof(char *) * (argc - optind)));
        lexcfiles
            = static_cast<FILE **>(malloc(sizeof(char *) * (argc - optind)));
        while (optind < argc)
        {
...
lexc_streams(LexcCompiler &lexc, HfstOutputStream &outstream)
{
    ...
    if (0 == res)
    {
        if (lexccount == 1)
        {
            hfst_error(
                EXIT_FAILURE, 0,
                "The file %s did not compile cleanly.\n"
...
main(int argc, char **argv)
{
    ...
    auto outstream
        = (outfile != stdout)
              ? std::make_unique<HfstOutputStream>(outfilename, format)
              : std::make_unique<HfstOutputStream>(format);
