...
/* Used by the tests. */
bool
compare_alphabets(const HfstTransducer &t1, const HfstTransducer &t2)
{
    ...
    if (not retval)
    {
        for (StringSet::const_iterator it = alpha1.begin(); it != alpha1.end();
             it++)
        {
            std::cerr << *it << std::endl;
        }
        std::cerr << "--" << std::endl;
        for (StringSet::const_iterator it = alpha2.begin(); it != alpha1.end();
             it++)
        {
            std::cerr << *it << std::endl;
        }
...
bool
compare_string_vectors(const StringVector &v1, const StringVector &v2,
...
bool
do_hfst_lookup_paths_contain(const HfstOneLevelPaths &results,
...
bool
do_results_contain(const HfstTwoLevelPaths &paths, const std::string &istring,
                   const std::string &ostring, float weight = 0,
                   bool test_path_weight = false)
{
...
void
print_string_vector(const StringVector &sv)
{
...
void
print_lookup_path(const HfstOneLevelPath &path)
{
...
void
print_lookup_paths(const HfstOneLevelPaths &paths)
{
...
bool
modify_transitions(const StringPair &sp, StringPairSet &sps)
{
...
main(int argc, char **argv)
{
    ...
    const ImplementationType types[] = { SFST_TYPE, TROPICAL_OPENFST_TYPE,
                                         /*LOG_OPENFST_TYPE,*/
                                         FOMA_TYPE };
    ...
    for (unsigned int i = 0; i < TYPES_SIZE; i++)
    {
        ...
        try
        {
            ...
            /* Functions extract_paths and extract_paths_fd. */
            {
                ...
                animals.extract_paths(results, 3, /* max_num */
                                      0 /* cycles */);
                     ...
                     it != results.end(); it++)
                {
                    ...
                    assert(expected_results.find(sp)
                ...
                /* Functions is_lookup_infinitely_ambiguous, lookup and
                 * lookup_fd. */
                ...
                // FATAL: EncodeMapper: Weight-encoded arc has non-trivial
                // weight
                if (types[i] != LOG_OPENFST_TYPE)
                {
                    ...
                    HfstTransducer hippopotamus2(
                        "hippopotamus", "hippopotamuses", tok, types[i]);
                ...
                assert(
                    not animals_ol.is_lookup_infinitely_ambiguous(lookup_cat));
                assert(
                    not animals_ol.is_lookup_infinitely_ambiguous(lookup_dog));
                ...
                results_hippopotamus
                    = animals_ol.lookup(lookup_hippopotamus, limit);
                ...
                // if type is LOG_OPENFST_TYPE:
                // FATAL: SingleShortestPath: Weight needs to have the path
                // property and be right distributive: log
                if (types[i] != LOG_OPENFST_TYPE)
                {
                    ...
                    if (weighted)
                    {
                        assert(do_results_contain(results1, "hippopotamus",
                                                  "hippopotami", 1.2, true));
                    }
                    ...
                    if (weighted)
                    {
                        assert(do_results_contain(results2, "hippopotamus",
                                                  "hippopotami", 1.2, true)
                               && do_results_contain(results2, "hippopotamus",
                                                     "hippopotamuses", 1.4,
                                                     true));
                    }
                    ...
                    if (weighted)
                    {
                        assert(do_results_contain(results3, "hippopotamus",
                                                  "hippopotami", 1.2, true)
                               && do_results_contain(results3, "hippopotamus",
                                                     "hippopotamuses", 1.4,
                                                     true)
                               && do_results_contain(results3, "mouse", "mice",
                                                     1.7, true));
                    }
                    ...
                    if (weighted)
                    {
                        assert(do_results_contain(results4, "hippopotamus",
                                                  "hippopotami", 1.2, true)
                               && do_results_contain(results4, "hippopotamus",
                                                     "hippopotamuses", 1.4,
                                                     true)
                               && do_results_contain(results4, "mouse", "mice",
                                                     1.7, true)
                               && do_results_contain(results4, "dog", "dogs",
                                                     2.5, true));
                    }
                    ...
                    if (weighted)
                    {
                        assert(do_results_contain(results5, "hippopotamus",
                                                  "hippopotami", 1.2, true)
                               && do_results_contain(results5, "hippopotamus",
                                                     "hippopotamuses", 1.4,
                                                     true)
                               && do_results_contain(results5, "mouse", "mice",
                                                     1.7, true)
                               && do_results_contain(results5, "dog", "dogs",
                                                     2.5, true)
                               && do_results_contain(results5, "cat", "cats",
                                                     3.0, true));
                    }
                }
            ...
            /* Functions insert_freely. */
            {
                ...
                HfstTransducer unk2unk("@_UNKNOWN_SYMBOL_@",
                                       "@_UNKNOWN_SYMBOL_@", types[i]);
                ...
                HfstTransducer unk2unk_("@_UNKNOWN_SYMBOL_@",
                                        "@_UNKNOWN_SYMBOL_@", types[i]);
        ...
        /* Function push_weights. */
        {
            if (types[i] == TROPICAL_OPENFST_TYPE)
            {
                ...
                try
                {
                    hfst::implementations::HfstBasicTransitions transitions
                        = t_initial[0];
        ...
        /* Functions substitute. */
        {
            ...
            t2.substitute(StringPair("a", "a"),
                          StringPair("a", "a")); // special
            t2.substitute(StringPair("foo", "bar"),
                          StringPair("f", "b")); // cases
...

        {
            ...
            HfstTransducer id2id(internal_identity, internal_identity,
                                 types[i]);
...

            {
                ...
                assert(id2id_copy.get_alphabet()
                       == id2id_copy2.get_alphabet());
            }
            ...
            /* Test that binary functions work when the argument and the
               calling object are the same. */
            {
