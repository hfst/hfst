...
{
bool
MultiCharSymbolTrie::is_end_of_string(const char *p) const
{
    return *(p + 1) == 0;
}

void
MultiCharSymbolTrie::set_symbol_end(const char *p)
{
    is_leaf[(unsigned char)(*p)] = true;
}

bool
MultiCharSymbolTrie::is_symbol_end(const char *p) const
{
    return is_leaf[(unsigned char)(*p)];
}

void
MultiCharSymbolTrie::init_symbol_rests(const char *p)
{
...
void
MultiCharSymbolTrie::add_symbol_rest(const char *p)
{
...
void
MultiCharSymbolTrie::add(const char *p)
{
...
const char *
MultiCharSymbolTrie::find(const char *p) const
{
...
bool
HfstTokenizer::is_skip_symbol(hfst::String &s) const
{
...
StringPairVector
HfstTokenizer::tokenize_space_separated(const std::string &str)
{
                        ...
                        const std::string &output_string) const
{
    ...
    if (input_spv.size() < output_spv.size())
    {
        ...
        for (StringPairVector::iterator it = input_spv.begin();
             it != input_spv.end(); ++it)
        {
            spv.push_back(StringPair(it->first, jt->first));
            ++jt;
        }
        ...
        const std::pair<std::string, std::string> &symbol_pair)) const
{
    ...
    while (it != input_spv.end() || jt != output_spv.end())
    {
        ...
        if (it == input_spv.end())
        {
            if (FdOperation::is_diacritic(
                    jt->first)) // copy diacritic to other side
            {
        ...
        {
            if (FdOperation::is_diacritic(
                    it->first)) // copy diacritic to other side
            {
        ...
        else
        {
            ...
            if ((!FdOperation::is_diacritic(it->first)
                 && !FdOperation::is_diacritic(jt->first))
                || *it == *jt)
            {
