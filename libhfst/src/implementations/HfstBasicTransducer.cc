...
HfstBasicTransducer::states() const
{
...
const HfstBasicStates &
HfstBasicTransducer::states_and_transitions() const
{
    return state_vector;
}

HfstBasicStates &
HfstBasicTransducer::states_and_transitions()
{
    return state_vector;
}
    ...
    const hfst::HfstTransducer &transducer)
{
    HfstBasicTransducer *fsm
        = ConversionFunctions::hfst_transducer_to_hfst_basic_transducer(
            transducer);
...
/* Add epsilon, unknown and identity symbols to the alphabet
   \a alpha. */
void
HfstBasicTransducer::initialize_alphabet(
    HfstBasicTransducer::HfstAlphabet &alpha)
{
...
bool
HfstBasicTransducer::check_alphabet()
{
    for (auto &it : *this)
    {
...
void
HfstBasicTransducer::print_alphabet() const
{
...
unsigned int
HfstBasicTransducer::get_symbol_number(const HfstSymbol &symbol) const
{
...
void
HfstBasicTransducer::initialize_state_vector(unsigned int number_of_states)
{
...
/* For internal optimization: Reserve space for
   ...
   \a state_number. */
void
HfstBasicTransducer::initialize_transition_vector(
...
void
HfstBasicTransducer::add_symbol_to_alphabet(const HfstSymbol &symbol)
{
...
void
HfstBasicTransducer::remove_symbol_from_alphabet(const HfstSymbol &symbol)
{
    alphabet.erase(symbol);
}

void
HfstBasicTransducer::remove_symbols_from_alphabet(const HfstSymbolSet &symbols)
{
...
/** @brief Same as #add_symbol_to_alphabet for each symbol in
    \a symbols. */
void
HfstBasicTransducer::add_symbols_to_alphabet(const HfstSymbolSet &symbols)
{
    for (const auto &symbol : symbols)
    {
        alphabet.insert(symbol);
...
void
HfstBasicTransducer::add_symbols_to_alphabet(const HfstSymbolPairSet &symbols)
{
    for (const auto &symbol : symbols)
    {
...
void
HfstBasicTransducer::prune_alphabet_after_substitution(
...
HfstBasicTransducer::symbols_used()
{
...
void
HfstBasicTransducer::prune_alphabet(bool force /*=true*/)
{
    ...
    bool unknowns_or_identities_used
        = ((symbols_found.find("@_UNKNOWN_SYMBOL_@") != symbols_found.end())
           || (symbols_found.find("@_IDENTITY_SYMBOL_@")
               != symbols_found.end()));
...
const HfstBasicTransducer::HfstAlphabet &
HfstBasicTransducer::get_alphabet() const
{
    return alphabet;
}

StringPairSet
HfstBasicTransducer::get_transition_pairs() const
{
    ...
    for (const auto &it : *this)
    {
        for (const auto &tr_it : it)
        {
            const auto &data = tr_it.get_transition_data();
            retval.insert(
                StringPair(data.get_input_symbol(), data.get_output_symbol()));
        }
    }
    return retval;
}
...
StringSet
HfstBasicTransducer::get_input_symbols() const
{
...
StringSet
HfstBasicTransducer::get_output_symbols() const
{
...
HfstState
HfstBasicTransducer::add_state(void)
{
    HfstBasicTransitions tr;
    state_vector.push_back(tr);
...
added to the graph if they did not exist before.
    @return \a s*/
HfstState
HfstBasicTransducer::add_state(HfstState s)
{
...
HfstState
HfstBasicTransducer::get_max_state() const
{
    return (HfstState)(state_vector.size() - 1);
}
...
void
HfstBasicTransducer::add_transition(HfstState s,
                                    const HfstBasicTransition &transition,
                                    bool add_symbols_to_alphabet /*=true*/)
{

    HfstTropicalTransducerTransitionData data
        = transition.get_transition_data();
...
void
HfstBasicTransducer::remove_transition(
    HfstState s, const HfstBasicTransition &transition,
...
bool
HfstBasicTransducer::is_final_state(HfstState s) const
{
...
HfstBasicTransducer::get_final_weight(HfstState s) const
{
...
void
HfstBasicTransducer::set_final_weight(
    HfstState s,
...
void
HfstBasicTransducer::remove_final_weight(HfstState s)
{
...
HfstBasicTransducer::sort_arcs(void)
{
    ...
    {
        std::sort<HfstBasicTransitions::iterator>(transitions.begin(),
                                                  transitions.end());
    }
    return *this;
}
...
/** @brief Get a const iterator to the beginning of
    states in the graph. */
HfstBasicTransducer::const_iterator
HfstBasicTransducer::begin() const
{
    return state_vector.begin();
}
...
/** @brief Get the set of transitions of state \a s in this graph.
    ...
    is thrown.
*/
const HfstBasicTransitions &
HfstBasicTransducer::operator[](HfstState s) const
{
 ...
 */
const HfstBasicTransitions &
HfstBasicTransducer::transitions(HfstState s) const
{
...
HfstBasicTransducer::transitions(HfstState s)
{
    if (s >= state_vector.size())
    {
        HFST_THROW(StateIndexOutOfBoundsException);
...
void
HfstBasicTransducer::swap_state_numbers(HfstState s1, HfstState s2)
{
    ...
    for (auto &it : *this)
    {
        ...
        for (unsigned int i = 0; i < it.size(); i++)
        {
            ...
            {
                HfstBasicTransition tr(new_target, tr_it.get_input_symbol(),
    ...
    {
        HfstTropicalTransducerTransitionData::WeightType s1_weight
            = s1_it->second;
...
void
HfstBasicTransducer::write_weight(FILE *file, float weight)
{
...
void
HfstBasicTransducer::write_weight(std::ostream &os, float weight)
{
    // if (weight == 0) // avoid unnecessary 0.000000's
    //   os << 0;
    // else
...
void
HfstBasicTransducer::xfstize(std::string &symbol)
{
...
void
HfstBasicTransducer::xfstize_symbol(std::string &symbol)
{
    xfstize(symbol);
...
void
HfstBasicTransducer::print_xfst_state(std::ostream &os, HfstState state)
{
...
void
HfstBasicTransducer::print_xfst_state(FILE *file, HfstState state)
{
    if (state == INITIAL_STATE)
    {
        fprintf(file, "S");
...
void
HfstBasicTransducer::print_xfst_arc(std::ostream &os,
                                    HfstTropicalTransducerTransitionData data)
{
...
void
HfstBasicTransducer::print_xfst_arc(FILE *file,
                                    HfstTropicalTransducerTransitionData data)
{
    if (data.get_input_symbol() != data.get_output_symbol())
    {
...
void
HfstBasicTransducer::write_in_xfst_format(std::ostream &os,
...
std::string
HfstBasicTransducer::prologize_symbol(const std::string &symbol)
{
    if (symbol == "0")
        return "%0";
    if (symbol == "?")
...
std::string
HfstBasicTransducer::deprologize_symbol(const std::string &symbol)
{
    if (symbol == "%0")
        return "0";
    if (symbol == "%?")
...
void
HfstBasicTransducer::print_prolog_arc_symbols(
    FILE *file, HfstTropicalTransducerTransitionData data)
{
...
void
HfstBasicTransducer::print_prolog_arc_symbols(
    std::ostream &os, HfstTropicalTransducerTransitionData data)
{
    std::string symbol = prologize_symbol(data.get_input_symbol());
    os << "\"" << symbol << "\"";
...
/** @brief Write the graph in prolog format to FILE \a file.
    \a write_weights defines whether weights are printed (todo). */
void
HfstBasicTransducer::write_in_prolog_format(FILE *file,
                                            ...
                                            bool write_weights /*=true*/)
{
    ...
    for (const auto &it : alphabet)
    {
        if (symbols_used_.find(it) == symbols_used_.end())
        {
            fprintf(file, "symbol(%s, \"%s\").\n", identifier,
                    prologize_symbol(it.c_str()).c_str());
        }
    }
    ...
    for (auto &it : *this)
    {
        for (const auto &tr_it : it)
        {
            fprintf(file, "arc(%s, %i, %i, ", identifier, source_state,
...
/** @brief Write the graph in prolog format to ostream \a os.
    \a write_weights defines whether weights are printed (todo). */
void
HfstBasicTransducer::write_in_prolog_format(std::ostream &os,
                                            const std::string &name,
                                            bool write_weights /*=true*/)
{
    unsigned int source_state = 0;
    ...
    for (const auto &it : alphabet)
    {
        if (symbols_used_.find(it) == symbols_used_.end())
        {
            os << "symbol(" << name << ", \"" << prologize_symbol(it) << "\")."
               << std::endl;
        }
    }
...
// Else, return false.
bool
HfstBasicTransducer::strip_quotes_from_both_sides(std::string &str)
{
    if (str.size() < 3)
        return false;
...
// Else, return false.
bool
HfstBasicTransducer::strip_ending_parenthesis_and_comma(std::string &str)
{
    if (str.size() < 3)
        return false;
...
bool
HfstBasicTransducer::parse_prolog_network_line(const std::string &line,
                                               HfstBasicTransducer &graph)
{
    // 'network(NAME).'
    char namearr[100];
...
HfstBasicTransducer::get_positions_of_unescaped_char(const std::string &str,
                                                     char c, char esc)
{
...
bool
HfstBasicTransducer::get_prolog_arc_symbols(const std::string &str,
                                            std::string &isymbol,
                                            std::string &osymbol)
{
    ...
    {
        if (quote_positions[0] != 0 || quote_positions[1] != str.length() - 1)
            return false; // extra characters outside quotes
    }
    // "foo":"bar"
    else if (quote_positions.size() == 4)
    {
        if (quote_positions[0] != 0 || quote_positions[3] != str.length() - 1)
        {
            return false; // extra characters outside quotes
        }
    ...
    {
        // "foo" -> foo
        std::string symbol(str, quote_positions[0] + 1,
                           quote_positions[1] - quote_positions[0] - 1);
    ...
    {
        // "foo" -> foo, "bar" -> bar
        std::string insymbol(str, quote_positions[0] + 1,
                             quote_positions[1] - quote_positions[0] - 1);
        std::string outsymbol(str, quote_positions[2] + 1,
                              quote_positions[3] - quote_positions[2] - 1);
...
bool
HfstBasicTransducer::extract_weight(std::string &symbol, float &weight)
{
    ...
    else if (last_double_quote + 2 == last_space
             && last_space < symbol.size() - 1) // + 2 because of the comma
    {
...
bool
HfstBasicTransducer::parse_prolog_arc_line(const std::string &line,
                                           HfstBasicTransducer &graph)
{
    ...
    graph.add_transition(
        source, HfstBasicTransition(target, isymbol, osymbol, weight));
    return true;
}

bool
HfstBasicTransducer::parse_prolog_final_line(const std::string &line,
                                             HfstBasicTransducer &graph)
{
    ...
    {
        int n
            = sscanf(line.c_str(), "final(%[^,], %[^)]).", namestr, finalstr);
        if (n != 2)
        {
            return false;
        }
    ...
    {
        int n = sscanf(line.c_str(), "final(%[^,], %[^,], %[^)]).", namestr,
                       finalstr, weightstr);
        if (n != 3)
        {
            return false;
...
bool
HfstBasicTransducer::parse_prolog_symbol_line(const std::string &line,
                                              HfstBasicTransducer &graph)
{
    // 'symbol(NAME, "foo").'
    char namearr[100];
...
std::string
HfstBasicTransducer::strip_newlines(std::string &str)
{
...
std::string
HfstBasicTransducer::get_stripped_line(std::istream &is, FILE *file,
...
HfstBasicTransducer
HfstBasicTransducer::read_in_prolog_format(std::istream &is, FILE *file,
                                           unsigned int &linecount)
{
...
HfstBasicTransducer
HfstBasicTransducer::read_in_prolog_format(std::istream &is,
                                           unsigned int &linecount)
{
    return read_in_prolog_format(is, NULL /* a dummy variable */, linecount);
}

HfstBasicTransducer
HfstBasicTransducer::read_in_prolog_format(FILE *file, unsigned int &linecount)
{
    return read_in_prolog_format(std::cin /* a dummy variable */, file,
                                 linecount);
}
...
/** @brief Write the graph in xfst text format to FILE \a file.
    \a write_weights defines whether weights are printed (todo). */
void
HfstBasicTransducer::write_in_xfst_format(FILE *file,
                                          bool write_weights /*=true*/)
{
    (void)write_weights;
    unsigned int source_state = 0;
...
void
HfstBasicTransducer::write_in_att_format(std::ostream &os,
                                         bool write_weights /*=true*/)
{
    unsigned int source_state = 0;
    for (auto &it : *this)
    {
        for (const auto &tr_it : it)
        {
            ...
            os << source_state << "\t" << tr_it.get_target_state() << "\t"
               << isymbol << "\t" << osymbol;
...
/** @brief Write the graph in AT&T format to FILE \a file.
    \a write_weights defines whether weights are printed. */
void
HfstBasicTransducer::write_in_att_format(FILE *file,
                                         bool write_weights /*=true*/)
{
    unsigned int source_state = 0;
    for (auto &it : *this)
    {
        for (const auto &tr_it : it)
        {
            ...
            fprintf(file, "%i\t%i\t%s\t%s", source_state,
                    tr_it.get_target_state(), isymbol.c_str(),
                    osymbol.c_str());
...
void
HfstBasicTransducer::write_in_att_format(char *ptr,
                                         bool write_weights /*=true*/)
{
    unsigned int source_state = 0;
    ...
    for (auto &it : *this)
    {
        for (const auto &tr_it : it)
        {
            ...
            cw = sprintf(ptr + cwt, "%i\t%i\t%s\t%s", source_state,
                         tr_it.get_target_state(), isymbol.c_str(),
                         osymbol.c_str());
            ...
            if (write_weights)
                cw = sprintf(ptr + cwt, "\t%f", data.get_weight());
            cwt = cwt + cw;
            cw = sprintf(ptr + cwt, "\n");
            cwt = cwt + cw;
        }
...
/** @brief Write the graph in AT&T format to FILE \a file using numbers
    ...
    \a write_weights defines whether weights are printed. */
void
HfstBasicTransducer::write_in_att_format_number(FILE *file,
                                                bool write_weights /*=true*/)
{
    unsigned int source_state = 0;
    for (auto &it : *this)
    {
        for (const auto &tr_it : it)
        {
            ...
            fprintf(file, "%i\t%i\t%i\t%i", source_state,
                    tr_it.get_target_state(), tr_it.get_input_number(),
            ...
            if (is_final_state(source_state))
            {
                fprintf(file, "%i", source_state);
                if (write_weights)
                    fprintf(file, "\t%f", get_final_weight(source_state));
                fprintf(file, "\n");
            }
        }
        source_state++;
                                  ...
                                  const std::string &epsilon_symbol)
{
    ...
    { // a transition line
        ...
        HfstBasicTransition tr(atoi(a2), input_symbol, output_symbol, weight);
...
void
HfstBasicTransducer::substitute_(HfstSymbol old_symbol, HfstSymbol new_symbol,
                                 ...
                                 bool output_side /*=true*/)
{
    ...
    for (iterator it = begin(); it != end(); it++)
    {
        ...
        for (unsigned int i = 0; i < it->size(); i++)
        {
            ...
            // current transition.
            HfstSymbol substituting_input_symbol = tr_it.get_input_symbol();
            HfstSymbol substituting_output_symbol = tr_it.get_output_symbol();

            // Whether a substitution will be performed.
            bool substitution_made = false;

            if (input_side && tr_it.get_input_symbol() == old_symbol)
            {
                substituting_input_symbol = new_symbol;
                substitution_made = true;
            }
            if (output_side && tr_it.get_output_symbol() == old_symbol)
            {
            ...
            if (substitution_made)
            {
                ...
                HfstBasicTransition tr(
                    tr_it.get_target_state(), substituting_input_symbol,
                    substituting_output_symbol, tr_it.get_weight());
...
void
HfstBasicTransducer::substitute_(const HfstNumberVector &substitutions,
    ...
    for (iterator it = begin(); it != end(); it++)
    {
        ...
        for (unsigned int i = 0; i < it->size(); i++)
        {
                ...
                || new_onumber != no_substitution)
            {
                ...
                // change the current transition accordingly.
                HfstBasicTransition tr(tr_it.get_target_state(), new_inumber,
                                       new_onumber, tr_it.get_weight(), false);
...
/* A function that performs in-place substitutions in the graph
   as defined in \a substitutions. */
void
HfstBasicTransducer::substitute_(
    const HfstNumberPairSubstitutions &substitutions)
{
    ...
    for (iterator it = begin(); it != end(); it++)
    {
        ...
        for (unsigned int i = 0; i < it->size(); i++)
        {
            ...
            HfstNumberPair old_number_pair(tr_it.get_input_number(),
                                           tr_it.get_output_number());
            ...
            if (subst_it != substitutions.end())
            {
                ...
                // change the current transition accordingly.
                HfstBasicTransition tr(tr_it.get_target_state(),
                                       new_input_number, new_output_number,
...
void
HfstBasicTransducer::remove_transitions(const HfstSymbolPair &sp)
{
    unsigned int in_match
        = HfstTropicalTransducerTransitionData::get_number(sp.first);
    unsigned int out_match
        = HfstTropicalTransducerTransitionData::get_number(sp.second);
...
/* A function that performs in-place-substitution in the graph. */
void
HfstBasicTransducer::substitute_(const HfstSymbolPair &old_sp,
    ...
    for (auto &it : *this)
    {
        ...
        for (unsigned int i = 0; i < it.size(); i++)
        {
                ...
                && tr_it.get_output_number() == old_output_number)
            {
                ...
                HfstSymbolPairSet::const_iterator IT = new_sps.begin();
...
/* A function that performs in-place-substitution in the graph. */
void
HfstBasicTransducer::substitute_(bool (*func)(const HfstSymbolPair &sp,
                                              HfstSymbolPairSet &sps))
{
    // ----- Go through all states. -----
    for (iterator it = begin(); it != end(); it++)
    {
        ...
        for (unsigned int i = 0; i < it->size(); i++)
        {
            ...
            HfstSymbolPair transition_symbol_pair(tr_it.get_input_symbol(),
                                                  tr_it.get_output_symbol());
            ...
            try
            {
                perform_substitution = (*func)(transition_symbol_pair,
            ...
            if (perform_substitution)
            {
                ...
                if (!HfstTropicalTransducerTransitionData::is_valid_symbol(
                        IT->first)
                    || !HfstTropicalTransducerTransitionData::is_valid_symbol(
                        IT->second))
                    HFST_THROW_MESSAGE(EmptyStringException,
                                       "HfstBasicTransducer::substitute");

                HfstBasicTransition tr(tr_it.get_target_state(), IT->first,
                                       IT->second, tr_it.get_weight());
                                ...
                                bool output_side /*=true*/)
{
    ...
    if (input_side && output_side)
    {
        ...
        if (!is_epsilon(old_symbol) && !is_unknown(old_symbol)
...
HfstBasicTransducer &
HfstBasicTransducer::substitute_symbols(
...
HfstBasicTransducer::substitute(const HfstSymbolSubstitutions &substitutions)
{
    ...
    size_t st = HfstTropicalTransducerTransitionData::get_max_number()
                + substitutions.size() + 1;
    unsigned int no_substitution = hfst::size_t_to_uint(st);

    substitutions_.resize(
        HfstTropicalTransducerTransitionData::get_max_number() + 1,
        no_substitution);
    for (const auto &substitution : substitutions)
    {
...
HfstBasicTransducer &
HfstBasicTransducer::substitute_symbol_pairs(
...
/** @brief Substitute all transitions \a sp with a set of transitions
    \a sps. */
HfstBasicTransducer &
HfstBasicTransducer::substitute(const HfstSymbolPair &sp,
...
/** @brief Substitute all transitions \a old_pair with
    \a new_pair. */
HfstBasicTransducer &
HfstBasicTransducer::substitute(const HfstSymbolPair &old_pair,
...
HfstBasicTransducer &
HfstBasicTransducer::substitute(bool (*func)(const HfstSymbolPair &sp,
                                             HfstSymbolPairSet &sps))
{
    substitute_(func);
    return *this;
...
hfst::implementations::HfstBasicTransducer &
HfstBasicTransducer::substitute(const HfstSymbolPair &sp,
                                const HfstBasicTransducer &graph)
{
    ...
    for (auto &it : *this)
    {
        ...
        std::vector<HfstBasicTransitions::iterator> old_transitions;
             ...
             tr_it != it.end(); tr_it++)
        {
                ...
                && data.get_output_symbol() == sp.second)
            {
                ...
                substitutions.push_back(substitution_data(
                    source_state, tr_it->get_target_state(), data.get_weight(),
...
void
HfstBasicTransducer::add_substitution(
    const HfstBasicTransducer::substitution_data &sub)
{
    // Epsilon transition to initial state of \a graph
    HfstState s = add_state();
    HfstBasicTransition epsilon_transition(
        s, HfstTropicalTransducerTransitionData::get_epsilon(),
        HfstTropicalTransducerTransitionData::get_epsilon(), sub.weight);
    ...
    for (const auto &it : *graph)
    {
        for (const auto &tr_it : it)
        {
            ...
            HfstBasicTransition transition(
                tr_it.get_target_state() + offset, data.get_input_symbol(),
                data.get_output_symbol(), data.get_weight());
    ...
    for (auto it : graph->final_weight_map)
    {
        HfstBasicTransition epsilon_transition(
            ...
            HfstTropicalTransducerTransitionData::get_epsilon(), it.second);
        add_transition(it.first + offset, epsilon_transition);
    }
}

std::string
HfstBasicTransducer::weight2marker(float weight)
{
    std::ostringstream o;
    o << weight;
...
HfstBasicTransducer &
HfstBasicTransducer::substitute_weights_with_markers()
{
    ...
    for (size_t state = 0; state < limit; state++)
    {
        ...
        std::stack<HfstBasicTransitions::iterator> old_transitions;
             ...
             tr_it != state_vector[state].end(); tr_it++)
        {
            ...
            {
                // schedule a substitution
                new_transitions.push_back(HfstBasicTransition(
                    tr_it->get_target_state(), data.get_input_symbol(),
                    data.get_output_symbol(), data.get_weight()));
                // schedule the old transition to be deleted
        ...
        for (auto &IT : new_transitions)
        {
            ...
            HfstBasicTransition marker_transition(IT.get_target_state(),
                                                  marker, marker, 0);
            HfstBasicTransition new_transition(
                new_state, IT.get_input_symbol(), IT.get_output_symbol(), 0);
    ...
    for (auto &fin_it : final_weight_map)
    {
        if (fin_it.second != 0)
        {
            ...
            HfstBasicTransition epsilon_transition(new_state, marker, marker,
                                                   0);
...
HfstBasicTransducer &
HfstBasicTransducer::substitute(SubstMap &substitution_map, bool harmonize)
{
    ...
    for (auto &it : *this)
    {
             ...
             tr_it != it.end(); tr_it++)
        {
            ...
            {
                std::string msg("symbol to be substituted must not occur only "
                                "on one side of transition");
                HFST_THROW_MESSAGE(HfstException, msg);
            }
            else
            {
                // schedule a substitution
                substitution_data sd(source_state, tr_it->get_target_state(),
                                     data.get_weight(),
                                     &(map_it_input->second));
...
bool
HfstBasicTransducer::marker2weight(const std::string &str, float &weight)
{
    if (str.size() < 3)
        return false;
...
HfstBasicTransducer &
HfstBasicTransducer::substitute_markers_with_weights()
{
    ...
    for (StringSet::iterator it = alphabet.begin(); it != alphabet.end(); it++)
    {
        float foo;
...
// aliases
HfstBasicTransducer &
HfstBasicTransducer::substitute_symbol(const std::string &old_symbol,
...
HfstBasicTransducer &
HfstBasicTransducer::substitute_symbol_pair(const StringPair &old_symbol_pair,
...
HfstBasicTransducer &
HfstBasicTransducer::substitute_symbol_pair_with_set(
...
HfstBasicTransducer &
HfstBasicTransducer::substitute_symbol_pair_with_transducer(
    const StringPair &symbol_pair, HfstBasicTransducer &transducer)
{
...
HfstBasicTransducer::insert_freely(
    const HfstSymbolPair &symbol_pair,
...
{
    HfstSymbol marker_this
        = HfstTropicalTransducerTransitionData::get_marker(alphabet);
    HfstSymbol marker_graph
        = HfstTropicalTransducerTransitionData::get_marker(alphabet);
...
HfstState
HfstBasicTransducer::disjunct(const StringPairVector &spv,
                              ...
                              HfstState s)
{
    ...
    while (it != spv.end())
    {
        ...
        if (!transition_found)
        {
            ...
            HfstBasicTransition transition(next_state, it->first, it->second,
                                           0);
...
bool
HfstBasicTransducer::is_special_symbol(const std::string &symbol)
{
    if (symbol.size() < 2)
        return false;
...
HfstBasicTransducer::complete()
{
    ...
    for (iterator it = begin(); it != end(); it++)
    {
        ...
        {
            if (symbols_present.find(alpha_it) == symbols_present.end()
...
StringSet
HfstBasicTransducer::get_flags() const
{
    StringSet flags;
    for (const auto &it : alphabet)
    {
...
bool
HfstBasicTransducer::purge_symbol(const std::string &symbol,
...
void
HfstBasicTransducer::flag_purge(const std::string &flag)
{
...
HfstBasicTransducer::topsort(SortDistance dist) const
{
    ...
    do
    {
        ...
        {
            TopSort.set_state_at_distance(new_state, current_distance + 1,
                                          (dist == MaximumDistance));
        }
        current_distance++;
...
int
HfstBasicTransducer::longest_path_size()
{
    // get topological maximum distance sort
    std::vector<std::set<HfstState> > states_sorted
        = this->topsort(MaximumDistance);
    // go through all sets of states in descending order
    size_t st = states_sorted.size();
    if (st > 0)
    {
        for (int distance = hfst::size_t_to_int(st - 1); distance >= 0;
             distance--)
        {
            const std::set<HfstState> &states
...
std::vector<unsigned int>
HfstBasicTransducer::path_sizes()
{
    std::vector<unsigned int> result;
    // get topological maximum distance sort
    std::vector<std::set<HfstState> > states_sorted
        = this->topsort(MinimumDistance);
    // go through all sets of states in descending order
    size_t st = states_sorted.size();
    if (st > 0)
    {
...
bool
HfstBasicTransducer::has_negative_epsilon_cycles(
    HfstState state, float total_weight,
    std::map<HfstState, float> &state_weights)
{
    ...
    const HfstBasicTransitions &transitions = this->operator[](state);
    for (const auto &transition : transitions)
    {
            ...
            && is_epsilon(transition.get_output_symbol()))
        {
            if (has_negative_epsilon_cycles(
                    ...
                    total_weight + transition.get_weight(), state_weights))
                return true;
        }
    }
...
HfstBasicTransducer::has_negative_epsilon_cycles()
{
    ...
    std::map<HfstState, float> state_weights;
    for (unsigned int state = INITIAL_STATE;
         state < (this->get_max_state() + 1); state++)
    {
...
bool
HfstBasicTransducer::is_infinitely_ambiguous(
    HfstState state, std::set<HfstState> &epsilon_path_states,
    std::vector<unsigned int> &states_handled)
{
    ...
    for (const auto &transition : transitions)
    {
        // (Diacritics are also treated as epsilons, although it might cause
        // false
        //  positive results, because loops with diacritics can be invalidated
        //  by other diacritics.)
        if (is_epsilon(transition.get_input_symbol())
            || FdOperation::is_diacritic(transition.get_input_symbol()))
        {
            ...
            if (is_infinitely_ambiguous(transition.get_target_state(),
                                        epsilon_path_states, states_handled))
            {
                return true;
            }
...
HfstBasicTransducer::is_infinitely_ambiguous()
{
    ...
    {
        if (is_infinitely_ambiguous(state, epsilon_path_states,
                                    states_handled))
            return true;
    }
    return false;
...
bool
HfstBasicTransducer::is_possible_flag(std::string symbol, StringVector &fds,
                                      bool obey_flags)
{
    if (FdOperation::is_diacritic(symbol))
    {
...
bool
HfstBasicTransducer::is_lookup_infinitely_ambiguous(
    const HfstOneLevelPath &s, unsigned int &index, HfstState state,
    std::set<HfstState> &epsilon_path_states, StringVector &fds,
    bool obey_flags)
{
    ...
    for (const auto &transition : transitions)
    {
        ...
        //  by other diacritics.)
        bool possible_flag
            = is_possible_flag(transition.get_input_symbol(), fds, obey_flags);
        if (is_epsilon(transition.get_input_symbol()) || possible_flag)
        {
            ...
            if (is_lookup_infinitely_ambiguous(
                    s, index, transition.get_target_state(),
        ...
        else if (!only_epsilons)
        {
            ...
            if (transition.get_input_symbol().compare(s.second.at(index)) == 0)
                continu = true;
            else if (((transition.get_input_symbol().compare(
                           "@_UNKNOWN_SYMBOL_@")
                       == 0)
                      || (transition.get_input_symbol().compare(
                              "@_IDENTITY_SYMBOL_@")
                          == 0))
            ...
            if (continu)
            {
                ...
                if (is_lookup_infinitely_ambiguous(
                        s, index, transition.get_target_state(), empty_set,
                        fds, obey_flags))
                {
                    return true;
                }
...
bool
HfstBasicTransducer::is_lookup_infinitely_ambiguous(const HfstOneLevelPath &s,
                                                    bool obey_flags /*=false*/)
{
    ...
    return is_lookup_infinitely_ambiguous(
        s, index, INITIAL_STATE, epsilon_path_states, fds, obey_flags);
}

bool
HfstBasicTransducer::is_lookup_infinitely_ambiguous(const StringVector &s,
                                                    bool obey_flags /*=false*/)
{
    std::set<HfstState> epsilon_path_states;
    epsilon_path_states.insert(0);
    ...
    return is_lookup_infinitely_ambiguous(
        path, index, INITIAL_STATE, epsilon_path_states, fds, obey_flags);
}

void
HfstBasicTransducer::push_back_to_two_level_path(
    HfstTwoLevelPath &path, const StringPair &sp, const float &weight,
...
void
HfstBasicTransducer::pop_back_from_two_level_path(
    HfstTwoLevelPath &path, const float &weight,
    StringVector *fds_so_far /*= NULL*/)
{
    if (fds_so_far != NULL)
    {
...
void
HfstBasicTransducer::add_to_results(HfstTwoLevelPaths &results,
                                    ...
                                    float *max_weight)
{
    ...
    else if (!(path_so_far.first
               > *max_weight)) // weight limitation not exceeded
    {
        results.insert(path_so_far);
    }
...
bool
HfstBasicTransducer::is_possible_transition(
    const HfstBasicTransition &transition, const StringVector &lookup_path,
    const unsigned int &lookup_index, const StringSet &alphabet,
    bool &input_symbol_consumed, StringVector *fds_so_far /*= NULL*/)
{
...
void
HfstBasicTransducer::lookup(const StringVector &lookup_path,
                            HfstTwoLevelPaths &results, HfstState state,
                            unsigned int lookup_index, // an iterator instead?
                            HfstTwoLevelPath &path_so_far, StringSet &alphabet,
                            HfstEpsilonHandler Eh, size_t max_epsilon_cycles,
                            ...
                            StringVector *flag_diacritic_path /*= NULL*/)
{
    ...
    if (lookup_index == lookup_path.size())
    {
        ...
        if (this->is_final_state(state))
        {
            ...
            add_to_results(results, path_so_far, this->get_final_weight(state),
                           max_weight);
        }
    }
    ...
    for (const auto &transition : transitions)
    {
        ...
        if (is_possible_transition(transition, lookup_path, lookup_index,
                                   alphabet, input_symbol_consumed,
                                   flag_diacritic_path))
        {
            ...
            push_back_to_two_level_path(path_so_far, StringPair(istr, ostr),
            ...
            lookup(lookup_path, results, transition.get_target_state(),
                   ...
                   max_epsilon_cycles, max_weight, max_number,
                   flag_diacritic_path);
            ...
            pop_back_from_two_level_path(path_so_far, transition.get_weight(),
                                         flag_diacritic_path);
        }
    }
}
                            ...
                            bool obey_flags /*= false*/)
{
    ...
    StringVector *flag_diacritic_path
        = (obey_flags) ? new StringVector() : NULL;
    ...
    if (max_epsilon_cycles != NULL)
    {
        ...
        lookup(lookup_path, results, state, lookup_index, path_so_far,
               alphabet, Eh, *max_epsilon_cycles, max_weight, max_number,
               flag_diacritic_path);
    }
    else
    {
        ...
        lookup(lookup_path, results, state, lookup_index, path_so_far,
               alphabet, Eh, 100000, max_weight, max_number,
               flag_diacritic_path);
    }
...
void
HfstBasicTransducer::check_regexp_state_for_cycle(
    HfstState s, const std::set<HfstState> &states_visited)
{
...
// Returns whether tr is "^]":"^]". If tr is not allowed, throws an error
// message.
bool
HfstBasicTransducer::check_regexp_transition_end(const HfstBasicTransition &tr,
                                                 bool input_side)
{
    ...
    else if ((input_side && is_special_symbol(istr))
             || (!input_side && is_special_symbol(ostr)))
    {
        throw "error: special symbol detected in compile-replace regular "
              "expression";
    }
    else
    {
    ...
    if ((input_side && ("^]" == istr)) || (!input_side && ("^]" == ostr)))
    {
        /*if (istr != ostr)
          {
            throw "error: ^] detected on only one side of transition inside
          compile-replace regular expression";
            }*/
        return true;
    }
    return false;
...
// [x:y]* "^]" (x and y cannot be "^]" or "^[") starting from state \a s. The
// resulting paths are stored in \a full_paths. \a path is used to keep track
// of each path so far. Weights are currently ignored.
void
HfstBasicTransducer::find_regexp_paths(
    HfstState s, std::set<HfstState> &states_visited,
    ...
    HfstReplacements &full_paths, bool input_side)
{
    ...
    // go through all transitions
    const HfstBasicTransitions &transitions = this->operator[](s);
    for (const auto &transition : transitions)
    {
        // closing bracket..
        if (check_regexp_transition_end(
                transition, input_side)) // throws error message if *it is not
                                         // a valid transition
        {
            // ..cannot lead to a state already visited..
            check_regexp_state_for_cycle(transition.get_target_state(),
                                         states_visited);
            // ..but else we can add the expression that it ends to the results
            path.push_back(std::pair<std::string, std::string>(
            ...
            path.pop_back(); // remove closing bracket as we are not going to
                             // proceed to next state
        }
        // add transition to path and call function again for its target state
        else
        {
            path.push_back(StringPair(transition.get_input_symbol(),
                                      transition.get_output_symbol()));
            find_regexp_paths(transition.get_target_state(), states_visited,
                              path, full_paths, input_side);
            path.pop_back();
        }
    }
    // all transitions gone through
...
// For each "^[":"^[" transition in state \a s, find continuing paths of form
// [x:y]* "^]":"^]" (where x and y can freely be any symbols except "^]" or
// "^[") and store those paths in \a full_paths vector where the first member
// of each element is the state where the ending "^]":"^]" transition leads to
// and the second element is a vector of transitions (i.e. string pairs)
// without the ending
// "^]":"^]" transition.
// An error is thrown if mismatched "^[" or "^]" symbols, special symbols
// (epsilon, unknown, identity), or loops are encountered on a regexp path.
// Final states are allowed on regexp paths as they are also allowed by Xerox
// tools. Weights are currently ignored.
void
HfstBasicTransducer::find_regexp_paths(
    HfstState s,
    std::vector<std::pair<HfstState,
...
// Find all subpaths of form "^[" [x:y]* "^]" (x and y cannot be "^[" or "^]")
// and return them. retval[start_state] == vector(pair(end_state,
// vector(pair(isymbol,osymbol) ) ) ) Weights are currently ignored.
HfstReplacementsMap
HfstBasicTransducer::find_replacements(bool input_side)
{
...
// Attach a copy of \a graph between states \a state1 and \a state2 with
// epsilon transitions. There will be an epsilon transition with weight zero
// from state \a state1 to the initial state of \a graph and one epsilon
// transition from each final state of
// \a graph to state \a state2 with a weight of that state's final weight.
// Final states of
// \a graph as well as its initial state are made normal non-final states when
// copying \a graph. Todo: copy alphabet? harmonize graphs?
void
HfstBasicTransducer::insert_transducer(HfstState state1, HfstState state2,
                                       const HfstBasicTransducer &graph)
{
    ...
    {
        HfstBasicTransition epsilon_transition(
            state2, HfstTropicalTransducerTransitionData::get_epsilon(),
            HfstTropicalTransducerTransitionData::get_epsilon(), it.second);
        add_transition(it.first + offset, epsilon_transition);
    }
    ...
    // Initial transition
    HfstBasicTransition epsilon_transition(
        offset, HfstTropicalTransducerTransitionData::get_epsilon(),
        HfstTropicalTransducerTransitionData::get_epsilon(), 0);
...
// Find target state corresponding to state pair \a target1, \a target2 in \a
// state_map and return that state. If not found, add a new state to \a
// intersection, add it to \a state_map and return it.
// \a was_new_state specifies whether a new state was added.
HfstState
HfstBasicTransducer::find_target_state(HfstState target1, HfstState target2,
                                       StateMap &state_map,
                                       HfstBasicTransducer &intersection,
                                       bool &was_new_state)
{
...
// Copy matching transition tr1/tr2 to state \a state in \a intersection and
// return the target state of that transition. Also make that state final, if
// needed.
HfstState
HfstBasicTransducer::handle_match(const HfstBasicTransducer &graph1,
                                  const HfstBasicTransition &tr1,
                                  const HfstBasicTransducer &graph2,
                                  const HfstBasicTransition &tr2,
                                  HfstBasicTransducer &intersection,
                                  HfstState state, StateMap &state_map)

{
    HfstState target1 = tr1.get_target_state();
    HfstState target2 = tr2.get_target_state();
    bool was_new_state = false;
    HfstState retval = find_target_state(target1, target2, state_map,
                                         intersection, was_new_state);
    ...
    intersection.add_transition(
        state,
        HfstBasicTransition(retval, tr1.get_input_symbol(),
                            tr1.get_output_symbol(), transition_weight));
    // For each new state added, check if the corresponding states in \a graph1
    // and \a graph2 are final. If they are, make the new state final with the
    // sum of final weights.
    if (was_new_state
        && (graph1.is_final_state(target1) && graph2.is_final_state(target2)))
    {
        float final_weight = graph1.get_final_weight(target1)
                             + graph2.get_final_weight(target2);
...
// @param state_map     State pairs from \a graph1 and \a graph2 mapped to
// states in \a intersection.
// @param agenda        States in \a intersection already handled or scheduled
// to be handled.
//
// @pre \a graph1 and \a graph2 must be arc-sorted (via sort_arcs()) to make
// transition matching faster.
// @pre \a graph1 and \a graph2 must be deterministic. (todo: handle equivalent
// transitions, maybe even epsilons?)
void
HfstBasicTransducer::find_matches(
    HfstBasicTransducer &graph1, HfstState state1, HfstBasicTransducer &graph2,
    HfstState state2, HfstBasicTransducer &intersection, HfstState state,
    StateMap &state_map, std::set<HfstState> &agenda)
{
    agenda.insert(state); // do not handle \a state twice
    HfstBasicTransitions &tr1
        = graph1.state_vector[state1]; // transitions of graph1
    HfstBasicTransitions &tr2
        = graph2.state_vector[state2]; // transitions of graph2
    ...
    // *** Go through all transitions of state \a state1 in \a graph1 and try
    // to find a match in transitions of state \a state2 in \a graph2. As
    // transitions are sorted, we know that if a match is found for tr1[i] in
    // tr2[j], a match for tr1[i+1] can be searched starting from tr2[j+1]. If
    // no match is found for tr1[i] in tr2 but tr2[j] is the first element that
    // is bigger than tr1[i], a match for tr1[i+1] can be searched starting
    // from tr2[j]. ***
    for (auto &transition1 : tr1)
    {
        // Transition data (input and output symbols) to be compared.
        const HfstTropicalTransducerTransitionData &transition_data1
            = transition1.get_transition_data();
        ...
        for (unsigned int j = start_search_from; j < tr2.size(); j++)
        {
            ...
            // Transition data (input and output symbols) to be compared.
            const HfstTropicalTransducerTransitionData &transition_data2
                = transition2.get_transition_data();
            // todo: input:output duplicates with different weights? (lower
            // weight is chosen always?)
            if (transition_data2.less_than_ignore_weight(transition_data1))
            {
                // no match found, continue searching
            }
            else if (transition_data1.less_than_ignore_weight(
                         transition_data2))
            {
            ...
            {
                // Match found, copy transitions and define target state in
                // intersection
                HfstState target
                    = handle_match(graph1, transition1, graph2, transition2,
                                   intersection, state, state_map);
                // Recursive call for target state, if it is not already on the
                // agenda.
                if (agenda.find(target) == agenda.end())
                {
                    find_matches(graph1, transition1.get_target_state(),
                                 graph2, transition2.get_target_state(),
                                 intersection, target, state_map, agenda);
                }
                // Start searching for next item in tr1 starting from next item
                // of tr2.
                start_search_from = j + 1;
                break;
            }
        }
        // --- A transition in tr1 compared for all possible transitions in
        // tr2, compare next transition. ---
    }
...
HfstBasicTransducer
HfstBasicTransducer::intersect(HfstBasicTransducer &graph1,
                               HfstBasicTransducer &graph2)
{
    HfstBasicTransducer retval;
    StateMap state_map;
    ...
    {
        float final_weight
            = std::min(graph1.get_final_weight(0), graph2.get_final_weight(0));
...
// Copy matching transition graph_tr/merger_tr to state \a result_state in \a
// result and return the target state of that transition. Also make that state
// final, if needed.
HfstState
HfstBasicTransducer::handle_non_list_match(
    const HfstBasicTransducer &graph,
    const HfstBasicTransition &graph_transition,
...

{
    ...
    bool was_new_state = false;
    HfstState retval = find_target_state(graph_target, merger_target,
                                         state_map, result, was_new_state);
    result.add_transition(
        result_state,
        HfstBasicTransition(retval, graph_transition.get_input_symbol(),
                            graph_transition.get_output_symbol(),
                            graph_transition.get_weight()));
    // For each new state added, check if the corresponding states in \a graph
    // and \a merger are final. If they are, make the new state final with the
    // sum of final weights.
    if (was_new_state
        && (graph.is_final_state(graph_target)
            && merger.is_final_state(merger_target)))
    {
        float final_weight = graph.get_final_weight(graph_target)
                             + merger.get_final_weight(merger_target);
...
// final, if needed.
HfstState
HfstBasicTransducer::handle_list_match(
    const HfstBasicTransducer &graph,
    const HfstBasicTransition &graph_transition,
    const HfstBasicTransducer &merger,
    const HfstBasicTransition &merger_transition, HfstBasicTransducer &result,
    HfstState result_state, StateMap &state_map,
    std::set<std::string> &markers_added)
{
    ...
    float transition_weight
        = graph_transition.get_weight() + merger_transition.get_weight();
    ...
    result.add_transition(
        result_state,
        HfstBasicTransition(
            extra_state, "@" + graph_transition.get_input_symbol() + "@",
            "@" + graph_transition.get_output_symbol() + "@", 0));
    markers_added.insert("@" + graph_transition.get_input_symbol() + "@");

    result.add_transition(
        extra_state /*result_state*/,
        HfstBasicTransition(retval, merger_transition.get_input_symbol(),
...
bool
HfstBasicTransducer::is_list_symbol(
    const HfstTropicalTransducerTransitionData &transition_data,
...
// @param state_map      State pairs from \a graph and \a merger mapped to
// states in \a result.
// @param agenda         States in \a result already handled or scheduled to be
// handled.
//
// @pre \a graph and \a merger must be arc-sorted (via sort_arcs()) to make
// transition matching faster.
// @pre \a graph and \a merger must be deterministic. (todo: handle equivalent
// transitions, maybe even epsilons?)
void
HfstBasicTransducer::find_matches_for_merge(
    HfstBasicTransducer &graph, HfstState graph_state,
    HfstBasicTransducer &merger, HfstState merger_state,
    HfstBasicTransducer &result, HfstState result_state, StateMap &state_map,
    std::set<HfstState> &agenda,
    const std::map<std::string, std::set<std::string> > &list_symbols,
    std::set<std::string> &markers_added)
{
    ...
    HfstBasicTransitions &graph_transitions
        = graph.state_vector[graph_state]; // transitions of graph
    HfstBasicTransitions &merger_transitions
        = merger.state_vector[merger_state]; // transitions of merger
    ...
    {
        const HfstTropicalTransducerTransitionData &graph_transition_data
            = graph_transition.get_transition_data();
        ...
        if (is_list_symbol(graph_transition_data, list_symbols))
        {
            const std::set<std::string> &symbol_list
                = list_symbols.find(graph_transition_data.get_input_symbol())
                      ->second;
            bool list_match_found = false;
            // Find all matches
            for (auto &merger_transition : merger_transitions)
            {
                const HfstTropicalTransducerTransitionData
                    &merger_transition_data
                    = merger_transition.get_transition_data();
                const std::string &isymbol
                    = merger_transition_data.get_input_symbol();
                const std::string &osymbol
                    = merger_transition_data.get_output_symbol();

                if (isymbol != osymbol)
                {
                    throw "find_matches_for_merge: input and output symbols "
                          "must be the same";
                }
                ...
                if (symbol_list.find(isymbol) != symbol_list.end())
                {
                    ...
                    HfstState target = handle_list_match(
                        graph, graph_transition, merger, merger_transition,
                        result, result_state, state_map, markers_added);
                    if (agenda.find(target) == agenda.end())
                    {
                        find_matches_for_merge(
                            graph, graph_transition.get_target_state(), merger,
                            merger_transition.get_target_state(), result,
                            target, state_map, agenda, list_symbols,
                            markers_added);
                    }
                }
            }
        ...
        // If symbol is not a list symbol or no match was found for it, copy
        // symbol as such We use merger_state as merger transition target state
        HfstState target = handle_non_list_match(graph, graph_transition,
                                                 merger, merger_state, result,
                                                 result_state, state_map);
        if (agenda.find(target) == agenda.end())
        {
            find_matches_for_merge(
                graph, graph_transition.get_target_state(), merger,
                /*merger_transition.get_target_state()*/ merger_state, result,
                target, state_map, agenda, list_symbols, markers_added);
        }
        // --- A transition in graph compared for all corresponding transitions
        // in merger, compare next transition. ---
    }
...
HfstBasicTransducer
HfstBasicTransducer::merge(
    HfstBasicTransducer &graph, HfstBasicTransducer &merger,
    const std::map<std::string, std::set<std::string> > &list_symbols,
    std::set<std::string> &markers_added)
{
    ...
    {
        float final_weight
            = graph.get_final_weight(0) + merger.get_final_weight(0);
        result.set_final_weight(0, final_weight);
    }

    try
    {
        find_matches_for_merge(graph, 0, merger, 0, result, 0, state_map,
                               agenda, list_symbols, markers_added);
    }
...
main(int argc, char *argv[])
{
    ...
    // g1.add_transition(0, HfstBasicTransition(1, "@_IDENTITY_SYMBOL_@",
    // "@_IDENTITY_SYMBOL_@", 0.1));
    ...
    // std::pair<std::string, std::string> p = std::pair<std::string,
    // std::string>("a", "a"); g1.substitute(p, subst);
