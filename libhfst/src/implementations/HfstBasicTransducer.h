...
class HfstBasicTransducer
{
  ...
  protected:
    ...
    unsigned int get_symbol_number(const HfstSymbol &symbol) const;

    /* For internal optimization: Reserve space for
       \a number_of_states states. */
    void initialize_state_vector(unsigned int number_of_states);

    /* For internal optimization: Reserve space for
       \a number_of_transitions transitions for state number
       \a state_number. */
    void initialize_transition_vector(unsigned int state_number,
  ...
  public:
    ...
    HFSTDLL void
    prune_alphabet_after_substitution(const std::set<unsigned int> &symbols);
    ...
    HFSTDLL void add_transition(HfstState s,
    ...
    HFSTDLL void remove_transition(HfstState s,
                                   const HfstBasicTransition &transition,
    ...
    HFSTDLL HfstTropicalTransducerTransitionData::WeightType
    get_final_weight(HfstState s) const;

    /** @brief Set the final weight of state \a s in this graph
        to \a weight.

        If the state does not exist, it is created. */
    HFSTDLL void set_final_weight(
        HfstState s,
        const HfstTropicalTransducerTransitionData::WeightType &weight);

    /** @brief Remove final weight from state \a s, i.e. make it a non-final
     * state. */
  ...
  protected:
    ...
    void print_xfst_arc(std::ostream &os,
  ...
  public:
    ...
    HFSTDLL void write_in_xfst_format(std::ostream &os,
    ...
    HFSTDLL static void
    print_prolog_arc_symbols(FILE *file,
                             HfstTropicalTransducerTransitionData data);

    HFSTDLL static void
    print_prolog_arc_symbols(std::ostream &os,
                             HfstTropicalTransducerTransitionData data);
    ...
    /** @brief Write the graph in prolog format to ostream \a os.
        \a write_weights defines whether weights are printed (todo). */
    HFSTDLL void write_in_prolog_format(std::ostream &os,
    ...
    HFSTDLL static bool parse_prolog_network_line(const std::string &line,
                                                  HfstBasicTransducer &graph);

    // Get positions of \a c in \a str. If \a esc is precedes
    // \a c, \a c is not included.
    HFSTDLL static std::vector<unsigned int>
    get_positions_of_unescaped_char(const std::string &str, char c, char esc);
    ...
    HFSTDLL static bool get_prolog_arc_symbols(const std::string &str,
                                               std::string &isymbol,
                                               std::string &osymbol);

    HFSTDLL static bool extract_weight(std::string &symbol, float &weight);

    HFSTDLL static bool parse_prolog_arc_line(const std::string &line,
                                              HfstBasicTransducer &graph);

    HFSTDLL static bool parse_prolog_final_line(const std::string &line,
                                                HfstBasicTransducer &graph);

    HFSTDLL static bool parse_prolog_symbol_line(const std::string &line,
                                                 HfstBasicTransducer &graph);
    ...
    HFSTDLL static std::string get_stripped_line(std::istream &is, FILE *file,
    ...
    read_in_prolog_format(std::istream &is, FILE *file,
                          unsigned int &linecount);

    HFSTDLL static HfstBasicTransducer
    read_in_prolog_format(std::istream &is, unsigned int &linecount);

    HFSTDLL static HfstBasicTransducer
    read_in_prolog_format(FILE *file, unsigned int &linecount);
    ...
    HFSTDLL void write_in_att_format(std::ostream &os,
                                     bool write_weights = true);
  ...
  protected:
    ...
    void substitute_(HfstSymbol old_symbol, HfstSymbol new_symbol,
  ...
  protected:
    ...
    /* A function that performs in-place-substitution in the graph. */
    void substitute_(bool (*func)(const HfstSymbolPair &sp,
  ...
  public:
    ...
    HFSTDLL HfstBasicTransducer &substitute(const HfstSymbol &old_symbol,
                                            const HfstSymbol &new_symbol,
                                            bool input_side = true,
                                            bool output_side = true);

    HFSTDLL HfstBasicTransducer &
    substitute_symbols(const HfstSymbolSubstitutions &substitutions);

    /** @brief Substitute all transitions as defined in \a substitutions */
    HfstBasicTransducer &
    substitute(const HfstSymbolSubstitutions &substitutions);

    HFSTDLL HfstBasicTransducer &
    substitute_symbol_pairs(const HfstSymbolPairSubstitutions &substitutions);
    ...
    */
    HFSTDLL HfstBasicTransducer &
    substitute(const HfstSymbolPairSubstitutions &substitutions);

    /** @brief Substitute all transitions \a sp with a set of transitions
        \a sps. */
    HFSTDLL HfstBasicTransducer &substitute(const HfstSymbolPair &sp,
                                            const HfstSymbolPairSet &sps);

    /** @brief Substitute all transitions \a old_pair with
        \a new_pair. */
    HFSTDLL HfstBasicTransducer &substitute(const HfstSymbolPair &old_pair,
    ...
    HFSTDLL HfstBasicTransducer &
    substitute(bool (*func)(const HfstSymbolPair &sp, HfstSymbolPairSet &sps));
    ...
    struct substitution_data
    {
        ...
        substitution_data(
            HfstState origin, HfstState target,
            HfstTropicalTransducerTransitionData::WeightType weight,
  ...
  public:
    ...
    */
    ...
    HFSTDLL HfstBasicTransducer &substitute(SubstMap &substitution_map,
                                            bool harmonize);
    ...
    // aliases
    HFSTDLL HfstBasicTransducer &
    substitute_symbol(const std::string &old_symbol,
                      const std::string &new_symbol, bool input_side = true,
                      bool output_side = true);

    HFSTDLL HfstBasicTransducer &
    substitute_symbol_pair(const StringPair &old_symbol_pair,
                           const StringPair &new_symbol_pair);

    HFSTDLL HfstBasicTransducer &substitute_symbol_pair_with_set(
        const StringPair &old_symbol_pair,
        const hfst::StringPairSet &new_symbol_pair_set);

    HFSTDLL HfstBasicTransducer &
    substitute_symbol_pair_with_transducer(const StringPair &symbol_pair,
                                           HfstBasicTransducer &transducer);
    ...
    HFSTDLL HfstBasicTransducer &
    insert_freely(const HfstSymbolPair &symbol_pair,
                  HfstTropicalTransducerTransitionData::WeightType weight);

    /** @brief Insert freely any number of any symbol in \a symbol_pairs in
        the graph with weight \a weight. */
    HFSTDLL HfstBasicTransducer &
    insert_freely(const HfstSymbolPairSet &symbol_pairs,
                  HfstTropicalTransducerTransitionData::WeightType weight);

    /** @brief Insert freely any number of \a graph in this
        graph. */
    HFSTDLL HfstBasicTransducer &
    insert_freely(const HfstBasicTransducer &graph);
  ...
  protected:
    ...
    HfstState disjunct(const StringPairVector &spv,
                       StringPairVector::const_iterator &it, HfstState s);
  ...
  public:
    ...
    */
    HFSTDLL HfstBasicTransducer &
    disjunct(const StringPairVector &spv,
             HfstTropicalTransducerTransitionData::WeightType weight);
    ...
    HFSTDLL bool purge_symbol(const std::string &symbol,
                              const std::string &flag);

    // Replace arcs in \a transducer that use flag \a flag with epsilon arcs
    // and remove \a flag from alphabet of \a transducer. If \a flag is the
    // empty string, replace/remove all flags.
    ...
    struct TopologicalSort
    {
        ...
        HFSTDLL void
        set_biggest_state_number(unsigned int biggest_state_number)
        {
        ...
        HFSTDLL void
        set_state_at_distance(HfstState state, unsigned int distance,
                              bool overwrite)
        {
            // see that 'state' does not exceed the maximum state number given
            // in initialization
            if (state > (distance_of_state.size() - 1))
            {
                std::cerr
                    << "ERROR in TopologicalSort::set_state_at_distance: "
                       "first argument ("
                    << state << ") is out of range (should be < "
                    << distance_of_state.size() << ")" << std::endl;
            }
            ...
            // if there was previous distance defined for 'state', erase it, if
            // needed
            int previous_distance = distance_of_state.at(state);
            if (previous_distance != -1 && previous_distance != (int)distance
                && overwrite)
            {
        ...
        HFSTDLL const std::set<HfstState> &
        get_states_at_distance(unsigned int distance)
        {
            // if there is nothing on index 'state',
            // push back empty sets of states up to index 'state', including
    ...
    */
    ...
    bool
    has_negative_epsilon_cycles(HfstState state, float total_weight,
                                std::map<HfstState, float> &state_weights);

    bool has_negative_epsilon_cycles();

    HFSTDLL bool
    is_infinitely_ambiguous(HfstState state,
    ...
    HFSTDLL bool
    is_lookup_infinitely_ambiguous(const HfstOneLevelPath &s,
    ...
    HFSTDLL static void
    push_back_to_two_level_path(HfstTwoLevelPath &path, const StringPair &sp,
                                const float &weight,
                                StringVector *fds_so_far = NULL);

    HFSTDLL static void
    pop_back_from_two_level_path(HfstTwoLevelPath &path, const float &weight,
                                 StringVector *fds_so_far = NULL);

    HFSTDLL static void add_to_results(HfstTwoLevelPaths &results,
    ...
    HFSTDLL void lookup(const StringVector &lookup_path,
                        ...
                        float *max_weight = NULL, int max_number = -1,
    ...
    HFSTDLL void
    check_regexp_state_for_cycle(HfstState s,
                                 const std::set<HfstState> &states_visited);

    // Returns whether tr is "^]":"^]". If tr is not allowed, throws an error
    // message.
    HFSTDLL bool check_regexp_transition_end(const HfstBasicTransition &tr,
                                             bool input_side);

    // If there is a "^[":"^[" transition leading to state \a s from some state
    // S and state S is included in \a states_visited and \a path and \a
    // full_paths are empty, this function can be used to find all (sub-)paths
    // of form [x:y]* "^]" (x and y cannot be "^]" or "^[") starting from state
    // \a s. The resulting paths are stored in \a full_paths. \a path is used
    // to keep track of each path so far. Weights are currently ignored.
    HFSTDLL void
    find_regexp_paths(HfstState s, std::set<HfstState> &states_visited,
                      std::vector<std::pair<std::string, std::string> > &path,
                      HfstReplacements &full_paths, bool input_side);

    // For each "^[":"^[" transition in state \a s, find continuing paths of
    // form [x:y]* "^]":"^]" (where x and y can freely be any symbols except
    // "^]" or "^[") and store those paths in \a full_paths vector where the
    // first member of each element is the state where the ending "^]":"^]"
    // transition leads to and the second element is a vector of transitions
    // (i.e. string pairs) without the ending
    // "^]":"^]" transition.
    ...
    // Final states are allowed on regexp paths as they are also allowed by
    // Xerox tools. Weights are currently ignored.
    HFSTDLL void find_regexp_paths(
        HfstState s,
        std::vector<std::pair<
            HfstState, std::vector<std::pair<std::string, std::string> > > >
            &full_paths,
        bool input_side);
    ...
    // epsilon transitions. There will be an epsilon transition with weight
    // zero from state \a state1 to the initial state of \a graph and one
    // epsilon transition from each final state of
    // \a graph to state \a state2 with a weight of that state's final weight.
    // Final states of
    // \a graph as well as its initial state are made normal non-final states
    // when copying \a graph. Todo: copy alphabet? harmonize graphs?
    HFSTDLL void insert_transducer(HfstState state1, HfstState state2,
                                   const HfstBasicTransducer &graph);

    typedef std::pair<HfstState, HfstState> StatePair;
    typedef std::map<StatePair, HfstState> StateMap;

    // Find target state corresponding to state pair \a target1, \a target2 in
    // \a state_map and return that state. If not found, add a new state to \a
    // intersection, add it to \a state_map and return it.
    // \a was_new_state specifies whether a new state was added.
    static HfstState find_target_state(HfstState target1, HfstState target2,
                                       StateMap &state_map,
                                       HfstBasicTransducer &intersection,
                                       bool &was_new_state);

    // A function used by find_matches.
    // Copy matching transition tr1/tr2 to state \a state in \a intersection
    // and return the target state of that transition. Also make that state
    // final, if needed.
    HFSTDLL static HfstState handle_match(const HfstBasicTransducer &graph1,
    ...
    // @param agenda        States in \a intersection already handled or
    // scheduled to be handled.
    //
    // @pre \a graph1 and \a graph2 must be arc-sorted (via sort_arcs()) to
    // make transition matching faster.
    // @pre \a graph1 and \a graph2 must be deterministic. (todo: handle
    // equivalent transitions, maybe even epsilons?)
    HFSTDLL static void
    find_matches(HfstBasicTransducer &graph1, HfstState state1,
                 HfstBasicTransducer &graph2, HfstState state2,
                 HfstBasicTransducer &intersection, HfstState state,
                 StateMap &state_map, std::set<HfstState> &agenda);

    HFSTDLL static HfstBasicTransducer intersect(HfstBasicTransducer &graph1,
                                                 HfstBasicTransducer &graph2);

    // A function used by find_matches_for_merge
    // Copy matching transition graph_tr/merger_tr to state \a result_state in
    // \a result and return the target state of that transition. Also make that
    // state final, if needed.
    HFSTDLL static HfstState
    handle_non_list_match(const HfstBasicTransducer &graph,
                          const HfstBasicTransition &graph_transition,
                          const HfstBasicTransducer &merger,
                          HfstState merger_target, HfstBasicTransducer &result,
    ...
    // state final, if needed.
    HFSTDLL static HfstState
    handle_list_match(const HfstBasicTransducer &graph,
                      const HfstBasicTransition &graph_transition,
                      const HfstBasicTransducer &merger,
                      const HfstBasicTransition &merger_transition,
                      HfstBasicTransducer &result, HfstState result_state,
                      StateMap &state_map,
                      std::set<std::string> &markers_added);

    HFSTDLL static bool is_list_symbol(
    ...
    // states in \a result.
    // @param agenda         States in \a result already handled or scheduled
    // to be handled.
    //
    // @pre \a graph and \a merger must be arc-sorted (via sort_arcs()) to make
    // transition matching faster.
    // @pre \a graph and \a merger must be deterministic. (todo: handle
    // equivalent transitions, maybe even epsilons?)
    ...
    HFSTDLL static HfstBasicTransducer
    merge(HfstBasicTransducer &graph, HfstBasicTransducer &merger,
          const std::map<std::string, std::set<std::string> > &list_symbols,
          std::set<std::string> &markers_added);
