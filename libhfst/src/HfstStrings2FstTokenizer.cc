...
void
HfstStrings2FstTokenizer::add_multichar_symbol(
...
void
HfstStrings2FstTokenizer::add_multichar_symbol_head(
    const std::string &multichar_symbol)
{
...
StringPairVector
HfstStrings2FstTokenizer::tokenize_pair_string(const std::string &str,
                                               bool spaces)
{
...
StringPairVector
HfstStrings2FstTokenizer::tokenize_string_pair(const std::string &str,
                                               bool spaces)
{
    StringVector tokenized_str;
    if (spaces)
    ...
    {
        return make_pair_vector(StringVector(tokenized_str.begin(), it),
                                StringVector(it + 1, tokenized_str.end()));
    }
}

StringPairVector
HfstStrings2FstTokenizer::make_pair_vector(const StringVector &v)
{
    ...
    for (StringVector::const_iterator it = v.begin(); it != v.end(); ++it)
    {
        if (!is_pair_input_symbol(it, v.end()))
        {
            ...
            symbol
                = (symbol.empty() || symbol == eps ? EPSILON_SYMBOL : symbol);
            spv.push_back(StringPair(symbol, symbol));
        }
        else
        {
            std::string input
                = (it->empty() || *it == eps ? EPSILON_SYMBOL : unescape(*it));
            ++(++it);
            std::string output
                = (it->empty() || *it == eps ? EPSILON_SYMBOL : unescape(*it));
...
StringPairVector
HfstStrings2FstTokenizer::make_pair_vector(const StringVector &input,
                                           const StringVector &output)
{
    ...
    while (input_it != input.end() && output_it != output.end())
    {
        ...
        spv.push_back(StringPair(
            input_symbol.empty() || input_symbol == eps ? EPSILON_SYMBOL
                                                        : input_symbol,
            output_symbol.empty() || output_symbol == eps ? EPSILON_SYMBOL
                                                          : output_symbol));
        ++input_it;
        ++output_it;
    }
    if (input_it == input.end())
    {
        ...
        {
            spv.push_back(StringPair(EPSILON_SYMBOL,
    ...
    else
    {
        ...
        {
            spv.push_back(StringPair(input_it->empty() || *input_it == eps
                                         ? EPSILON_SYMBOL
                                         : unescape(*input_it),
                                     EPSILON_SYMBOL));
        }
...
std::string
HfstStrings2FstTokenizer::unescape(std::string symbol)
{
    check_cols(symbol);
...
bool
HfstStrings2FstTokenizer::is_pair_input_symbol(
...
void
HfstStrings2FstTokenizer::check_cols(const std::string &symbol)
{
    if (!symbol.empty())
    {
...
int
HfstStrings2FstTokenizer::get_col_pos(const std::string &str)
{
    if (str.empty())
    {
        return -1;
...
void
test_ps(const std::string &input, hfst::HfstStrings2FstTokenizer &tokenizer,
        bool spaces)
{
...
void
test_sp(const std::string &input, hfst::HfstStrings2FstTokenizer &tokenizer,
        bool spaces)
{
    std::cout << "Tokenizing: " << input << std::endl;
    std::cout << "Tokenized:" << std::endl;
...
main(void)
{
    ...
    test_sp("@_EPS_@koira@_EPS_@@_EPS_@:##koira\\:NOM:SG##", tokenizer, false);
    test_sp("@_EPS_@koira@_EPS_@@_EPS_@:##koira\\\\:NOM:SG##", tokenizer,
            false);
    test_sp("@_EPS_@ k o i r  a @_EPS_@ @_EPS_@ : ## k o i   r  a :NOM:SG ##",
            tokenizer, true);
    test_sp(
        "@_EPS_@ k o i r  a \\  @_EPS_@ @_EPS_@ : ## k o i   r  a :NOM:SG ##",
        tokenizer, true);
}
