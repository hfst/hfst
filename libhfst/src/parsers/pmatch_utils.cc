...
void
zero_minimization_guard(void)
{
    minimization_guard_count = 0;
}

PmatchTransducerContainer *
make_minimization_guard(void)
{
...
bool
symbol_in_global_context(std::string &sym)
{
    return definitions.count(sym) != 0;
}

bool
symbol_in_local_context(std::string &sym)
{
...
PmatchObject *
symbol_from_global_context(std::string &sym)
{
...
PmatchObject *
symbol_from_local_context(std::string &sym)
{
...
bool
string_set_has_meta_arc(StringSet &ss)
{
...
bool
is_special(const std::string &symbol)
{
    if (symbol.size() < 3)
    {
        return false;
...
HfstTransducer *
add_pmatch_delimiters(HfstTransducer *regex)
{
    HfstTransducer *delimited_regex = new HfstTransducer(
        hfst::internal_epsilon, ENTRY_SYMBOL, regex->get_type());
    delimited_regex->concatenate(*regex);
    delimited_regex->concatenate(HfstTransducer(
        hfst::internal_epsilon, EXIT_SYMBOL, regex->get_type()));
    delete regex;
    return delimited_regex;
}

PmatchTransducerContainer *
make_end_tag(std::string tag)
{
    return epsilon_to_symbol_container("@PMATCH_ENDTAG_" + tag + "@");
}

PmatchTransducerContainer *
make_capture_tag(std::string tag)
{
    return epsilon_to_symbol_container("@PMATCH_CAPTURE_" + tag + "@");
}

PmatchTransducerContainer *
make_captured_tag(std::string tag)
{
    return epsilon_to_symbol_container("@PMATCH_CAPTURED_" + tag + "@");
}

PmatchObject *
make_with_tag_entry(std::string key, std::string value)
{
    return new PmatchString("@P.PMATCH_GLOBAL_" + key + "." + value + "@");
}

PmatchObject *
make_with_tag_exit(std::string key)
{
...
get_top_n(size_t n, std::vector<WordVector> &vecs,
            ...
            else
            {
                ...
                if (cosdist >= retval[i].second)
                {
                    ...
                    retval.insert(retval.begin() + i,
                                  std::pair<WordVector, WordVecFloat>(
                                      WordVector(*it), cosdist));
                    break;
                }
                else
...
std::vector<std::pair<WordVector, WordVecFloat> >
get_top_n_transformed(size_t n, const std::vector<WordVector> &vecs,
                      std::vector<WordVecFloat> plane_vec,
                      std::vector<WordVecFloat> comparison_point,
                      WordVecFloat translation_term, bool negative)
{
    std::vector<std::pair<WordVector, WordVecFloat> > retval;
         ...
         it != vecs.end(); ++it)
    {
        ...
        WordVecFloat transformed_vec_scaler
            = (translation_term
               - dot_product(transformed_vec.vector, plane_vec))
        ...
        WordVecFloat cosdist
            = 1
              - dot_product(transformed_vec.vector, comparison_point)
...
template <typename T>
std::vector<T>
pointwise_minus(std::vector<T> l, std::vector<T> r)
{
...
template <typename T>
std::vector<T>
pointwise_plus(std::vector<T> l, std::vector<T> r)
{
    std::vector<T> ret(l.size(), 0);
    for (size_t i = 0; i < l.size(); ++i)
    {
...
template <typename T>
std::vector<T>
pointwise_multiplication(T l, std::vector<T> r)
{
...
template <typename T>
T
dot_product(std::vector<T> l, std::vector<T> r)
{
    T ret = 0;
    for (size_t i = 0; i < l.size(); ++i)
    {
...
WordVecFloat
cosine_distance(WordVector left, WordVector right)
{
    ...
    WordVecFloat retval
        = 1.0
          - dot_product(left.vector, right.vector) / (left.norm * right.norm);
    return std::max(static_cast<WordVecFloat>(0.0), retval);
}

WordVecFloat
cosine_distance(std::vector<WordVecFloat> left,
                std::vector<WordVecFloat> right)
{
    WordVecFloat retval
        = 1.0 - dot_product(left, right) / (norm(left) * norm(right));
    return std::max(static_cast<WordVecFloat>(0.0), retval);
}

// Single-word Like()
PmatchObject *
compile_like_arc(std::string word, unsigned int nwords)
{
    ...
    std::vector<std::pair<WordVector, WordVecFloat> > top_n
        = get_top_n(nwords, word_vectors, this_word);
...
// the general case
PmatchObject *
compile_like_arc(std::string word1, std::string word2, unsigned int nwords,
                 bool is_negative)
{
    ...
    for (std::vector<WordVector>::iterator it = word_vectors.begin();
         (it != word_vectors.end()
          && (this_word1.word.empty() || this_word2.word.empty()));
         ++it)
    {
        if (word1 == it->word)
        {
    ...
    {
        // just one match
        pmatchwarning("only one match for arguments to Like() operation, "
                      "using nearest neighbours");
        WordVector this_word
            = (this_word1.word.empty() ? this_word2 : this_word1);
        std::vector<std::pair<WordVector, WordVecFloat> > top_n
            = get_top_n(nwords, word_vectors, this_word);
        HfstTokenizer tok;
        HfstTransducer *retval = new HfstTransducer(format);
        if (verbose)
        {
            std::cerr << "Inserting into Like(" << this_word.word
                      << "):" << std::endl;
        }
    ...
    {
        vector_similarity_projection_factor = strtod(
            variables["vector-similarity-projection-factor"].c_str(), NULL);
    }
    /*
    ...
    std::vector<WordVecFloat> B_minus_A
        = pointwise_minus(this_word1.vector, this_word2.vector);
    WordVecFloat hyperplane_translation_term
    ...
    if (is_negative == true)
    {
        if (verbose)
        {
            std::cerr << "Inserting into Unlike(" << this_word1.word << ", "
                      << this_word2.word << "):" << std::endl;
        }
        WordVecFloat comparison_scaler
            = (hyperplane_translation_term
               - dot_product(this_word1.vector, B_minus_A))
              / square_sum(B_minus_A);
        comparison_scaler *= vector_similarity_projection_factor;
        comparison_point = pointwise_minus(
            this_word1.vector,
            pointwise_multiplication(comparison_scaler, B_minus_A));
    }
    else
    {
        if (verbose)
        {
            std::cerr << "Inserting into Like(" << this_word1.word << ", "
                      << this_word2.word << "):" << std::endl;
        }
    ...
    std::vector<std::pair<WordVector, WordVecFloat> > top_n
        = get_top_n_transformed(nwords, word_vectors, B_minus_A,
                                comparison_point, hyperplane_translation_term,
                                is_negative);
    HfstTokenizer tok;
    HfstTransducer *retval = new HfstTransducer(format);
...
PmatchTransducerContainer *
make_counter(std::string name)
{
    return epsilon_to_symbol_container("@PMATCH_COUNTER_" + name + "@");
}

hfst::StringSet
get_non_special_alphabet(HfstTransducer *t)
{
...
HfstTransducer *
make_list(HfstTransducer *t, ImplementationType f)
{
    std::string arc = "@L.";
...
HfstTransducer *
make_exc_list(HfstTransducer *t, ImplementationType f)
{
    std::string arc = "@X.";
    hfst::StringSet alphabet = get_non_special_alphabet(t);
    for (hfst::StringSet::const_iterator it = alphabet.begin();
...
PmatchTransducerContainer *
epsilon_to_symbol_container(std::string s)
{
    HfstTransducer *tmp
        = new HfstTransducer(hfst::internal_epsilon, s, format);
...
char *
get_delimited(const char *s, char delim_left, char delim_right)
{
...
char *
get_delimited(const char *s, char delim)
{
    return get_delimited(s, delim, delim);
}

char *
get_escaped_delimited(const char *s, char delim_left, char delim_right)
{
    return unescape_delimited(get_delimited(s, delim_left, delim_right),
                              delim_right);
}
char *
get_escaped_delimited(const char *s, char delim)
{
    return unescape_delimited(get_delimited(s, delim, delim), delim);
}

char *
unescape_delimited(char *s, char delim)
{
    char *read = s;
    char *write = s;
    while (*read != '\0')
...
parse_quoted(const char *s)
{
    ...
    char *rv
        = static_cast<char *>(malloc(sizeof(char) * (strlen(quoted) + 1)));
    char *p = quoted;
    char *r = rv;
    while (*p != '\0')
    {
        ...
        else if (*p == '\\')
        {
            ...
            case '7':
                fprintf(stderr,
                        "*** PMATCH unimplemented: "
                        "parse octal escape in %s",
                        p);
                *r = '\0';
                p = p + 5;
                break;
...
unsigned int
next_utf8_to_codepoint(unsigned char **c)
{
...
std::string
codepoint_to_utf8(unsigned int codepoint)
{
    char buf[5];
...
parse_range(const char *s)
{
    ...
    while (**c != '\0')
    {
        ...
        *c += 1;
        ...
        {
            retval->disjunct(
                HfstTransducer(codepoint_to_utf8(codepoint1), format));
            ++codepoint1;
        }
    }
    free(orig_quoted);
...
string
expand_includes(const string &script)
{
    ...
    while (it != script.end())
    {
        ...
        {
            size_t terminating_quote_pos
                = script.find('"', it - script.begin() + 9);
            if (terminating_quote_pos != string::npos)
            {
                size_t filename_start_pos = it - script.begin() + 9;
                size_t filename_len
                    = terminating_quote_pos - filename_start_pos;
                string filepath = path_from_filename(
                    script.substr(filename_start_pos, filename_len).c_str());
                std::ifstream infile;
                infile.open(filepath.c_str());
                if (!infile.good())
                {
                    std::stringstream errstring;
                    errstring << "could not open file " << filepath
                              << " for @include\n";
...
compile(const string &pmatch, map<string, HfstTransducer *> &defs,
        ...
        bool do_include_cosine_distances, std::string includedir_)
{
    // lock here?
    init_globals();
         ...
         it != unsatisfied_insertions.end(); ++it)
    {
        ...
        {
            std::cerr << "Inserted transducer " << *it
                      << " was never defined!\n";
            return retval;
        }
    }
    if (hfst::pmatch::verbose)
    {
             ...
             ++defs_itr)
        {
            ...
            {
                std::cerr << "Warning: " << defs_itr->first
    ...
    else
    {
        if (definitions.size() == 0)
        {
            ...
            retval.insert(std::pair<std::string, hfst::HfstTransducer *>(
                "TOP", new HfstTransducer(format)));
        }
        else if (definitions.count("TOP") == 0)
        {
            std::cerr << "Pmatch compilation warning: regex or TOP was "
                         "undefined, using ";
            std::cerr << definitions.begin()->first << " as root\n";
            hfst::HfstTransducer *tmp
                = definitions.begin()->second->evaluate();
            tmp->minimize();
            tmp->set_name("TOP");
            retval.insert(
                std::pair<std::string, hfst::HfstTransducer *>("TOP", tmp));
        }
        else
        {
    ...
    {
        double duration
            = (clock() - hfst::pmatch::timer) / (double)CLOCKS_PER_SEC;
        hfst::pmatch::timer = clock();
        std::cerr << "Everything compiled and harmonized in " << duration
                  << " seconds\n";
    }
    ...
    if (allowed_initial_symbols.size() > 200)
    {
        if (hfst::pmatch::verbose)
        {
            std::cerr
                ...
                << allowed_initial_symbols.size() << std::endl;
        }
        initial_symbols_ok = false;
    }
    if (disallowed_initial_symbols.size() > 200)
    {
        if (hfst::pmatch::verbose)
        {
            std::cerr
                << "Not setting initial symbol list due to excess length: "
                << disallowed_initial_symbols.size() << std::endl;
        }
        initial_symbols_ok = false;
    }
    ...
    {
        variables["disallowed-initial-symbols"]
    ...
    if (variables["need-separators"] == "on")
    {
        ...
        {
            HfstTransducer whitespace_punct_context(
                *(get_utils()->latin1_whitespace_acceptor));
            whitespace_punct_context.disjunct(
                *(get_utils()->latin1_punct_acceptor));
            whitespace_punct_context.disjunct(
                HfstTransducer("@BOUNDARY@", format));
            HfstTransducer *top_with_boundaries = new HfstTransducer(
                hfst::internal_epsilon, LC_ENTRY_SYMBOL, format);
            top_with_boundaries->concatenate(whitespace_punct_context);
            top_with_boundaries->concatenate(HfstTransducer(
                hfst::internal_epsilon, LC_EXIT_SYMBOL, format));
            HfstTransducer RC(hfst::internal_epsilon, RC_ENTRY_SYMBOL, format);
            RC.concatenate(whitespace_punct_context);
            RC.concatenate(HfstTransducer(hfst::internal_epsilon,
                                          RC_EXIT_SYMBOL, format));
            ...
            if (hfst::pmatch::verbose)
            {
                double duration
                    = (clock() - hfst::pmatch::timer) / (double)CLOCKS_PER_SEC;
                hfst::pmatch::timer = clock();
                std::cerr << "Added automatic context separators in "
                          << duration << " seconds\n";
            }
        }
    }
...
std::string
get_size_info(HfstTransducer *net)
{
...
void
write_compilation_stack_indentation_to_err(void)
{
...
HfstTransducer *
read_text(std::string filename, ImplementationType type, bool spaced_text)
{
    std::ifstream infile;
    std::string line;
...
HfstTransducer *
read_spaced_text(std::string filename, ImplementationType type)
{
...
read_vec(std::string filename)
{
    ...
    if (binary_format)
    {
        ...
        while (infile.good() && words_read <= lexicon_size)
        {
            ...
            // foreseeable future
            wv.vector.assign((float *)vector_data.data(),
                             (float *)(vector_data.data() + vector_data_size));
    ...
    else
    {
        while (infile.good() && words_read <= lexicon_size)
        {
            ...
            while (std::string::npos
                   != (nextpos = line.find(separator, pos + 1)))
            {
                components.push_back(
                    strtod(line.substr(pos + 1, nextpos - pos).c_str(), NULL));
                pos = nextpos;
            }
            ...
            if (line.back() != separator)
            {
                ...
                components.push_back(
                    (float)strtod(line.substr(pos + 1).c_str(), NULL));
            }
#else
                components.push_back(
                    strtof(line.substr(pos + 1).c_str(), NULL));
            }
#endif
            if (word_vectors.size() != 0
                && word_vectors[0].vector.size() != components.size())
            {
                std::cerr << "pmatch warning: vector file " << filename
                          << " appears malformed\n  (reading line "
                          << words_read + 1 << ")\n";
    ...
    if (verbose)
    {
        ...
        std::cerr << "Read " << word_vectors.size()
                  << " vectors of dimensionality "
                  << word_vectors[0].vector.size() << std::endl;
    }
}

std::vector<std::vector<std::string> >
read_args(char *filename, unsigned int argcount)
{
    std::ifstream infile;
    std::string line;
        ...
        while (infile.good())
        {
            ...
            if (!line.empty())
            {
                ...
                do
                {
                    curpos = nextpos + 1;
                    nextpos
                        = hfst::size_t_to_int(line.find_first_of(" ", curpos));
                    current_tokens.push_back(
                        line.substr(curpos, nextpos - curpos));
                } while (nextpos != std::string::npos);
                if (current_tokens.size() != argcount)
                {
                    std::cerr << "Pmatch: line " << n << " in " << filename
                              << " contained " << current_tokens.size()
                              << " tokens, expected " << argcount << std::endl;
                }
                else
                {
...
HfstTransducer *
PmatchUtilityTransducers::make_latin1_acceptor(ImplementationType type)
{
    HfstTransducer *retval = make_latin1_alpha_acceptor();
    HfstTransducer *tmp = make_latin1_numeral_acceptor();
    retval->disjunct(*tmp);
    delete tmp;
    tmp = make_latin1_punct_acceptor();
    retval->disjunct(*tmp);
    delete tmp;
    tmp = make_latin1_whitespace_acceptor();
    retval->disjunct(*tmp);
    delete tmp;
    retval->minimize();
    return retval;
...
HfstTransducer *
PmatchUtilityTransducers::make_latin1_alpha_acceptor(ImplementationType type)
{
...
HfstTransducer *
PmatchUtilityTransducers::make_latin1_uppercase_acceptor(
    ImplementationType type)
{
...
HfstTransducer *
PmatchUtilityTransducers::make_combining_accent_acceptor(
    ImplementationType type)
{
...
HfstTransducer *
PmatchUtilityTransducers::make_latin1_numeral_acceptor(ImplementationType type)
{
    HfstTransducer *retval = new HfstTransducer(type);
...
HfstTransducer *
PmatchUtilityTransducers::make_latin1_punct_acceptor(ImplementationType type)
{
...
HfstTransducer *
PmatchUtilityTransducers::make_capify(ImplementationType type)
{
    HfstTransducer *retval = new HfstTransducer(type);
    HfstTokenizer tok;
    ...
    {
        retval->disjunct(
            HfstTransducer(latin1_lower[i], latin1_upper[i], tok, type));
    }
...
HfstTransducer *
PmatchUtilityTransducers::make_lowerfy(ImplementationType type)
{
    HfstTransducer *retval = new HfstTransducer(type);
    HfstTokenizer tok;
    for (size_t i = 0; i < array_len(latin1_upper); ++i)
    {
        retval->disjunct(
            HfstTransducer(latin1_upper[i], latin1_lower[i], tok, type));
    }
    HfstTransducer accents(*combining_accent_acceptor);
    accents.optionalize();
    retval->concatenate(accents);
...
HfstTransducer
PmatchUtilityTransducers::get_uppercase_acceptor_from_transducer(
    HfstTransducer &t)
{
...
HfstTransducer
PmatchUtilityTransducers::lowercaser_from_transducer(HfstTransducer &t)
{
    HfstTransducer lowercase(t.get_type());
    StringSet ss = t.get_alphabet();
...
HfstTransducer
PmatchUtilityTransducers::uppercaser_from_transducer(HfstTransducer &t)
{
    HfstTransducer uppercase(t.get_type());
    StringSet ss = t.get_alphabet();
    StringSet uppercases_seen;
...
HfstTransducer *
PmatchUtilityTransducers::cap(HfstTransducer &t, Side side, bool optional)
{
...
HfstTransducer *
PmatchUtilityTransducers::tolower(HfstTransducer &t, Side side, bool optional)
{
    bool orig_xerox_composition_value = hfst::get_xerox_composition();
    // This is to match flags in t with ?'s in "anything"
    hfst::set_xerox_composition(true);
...
HfstTransducer *
PmatchUtilityTransducers::toupper(HfstTransducer &t, Side side, bool optional)
{
...
HfstTransducer *
PmatchObject::evaluate(std::vector<PmatchObject *> args)
{
    if (args.size() == 0)
    {
        if (should_use_cache())
    ...
    {
        std::stringstream errstring;
        errstring << "Object " << name << " on line " << pmatchlineno
                  << " has no argument handling";
...
void
PmatchObject::expand_Ins_arcs(StringSet &ss)
{
                    ...
                    if (definitions.count(ins_name) != 0)
                    {
                        ...
                        if (def_insed_expressions.count(ins_name) != 0)
                        {
                            def_insed_expressions[ins_name]
                                ->collect_initial_symbols_into(allowed,
                                                               disallowed);
                        }
                        else
                        {
                        ...
                        if (allowed.size() != 0)
                        {
                            expanded_symbols.insert(allowed.begin(),
                                                    allowed.end());
                        }
                        else
                        {
...
StringSet
PmatchObject::get_real_initial_symbols(void)
{
...
StringSet
PmatchObject::get_real_initial_symbols_from_right(void)
{
    return StringSet();
}

StringSet
PmatchBinaryOperation::get_real_initial_symbols_from_right(void)
{
    return right->get_real_initial_symbols();
}

StringSet
PmatchObject::get_initial_symbols_from_unary_root(void)
{
    return StringSet();
}

StringSet
PmatchUnaryOperation::get_initial_symbols_from_unary_root(void)
{
    return root->get_real_initial_symbols();
}

bool
PmatchObject::is_left_concatenation_with_context(void)
{
    return false;
}

bool
PmatchBinaryOperation::is_left_concatenation_with_context(void)
{
    return op == Concatenate && left->is_context();
}

bool
PmatchObject::is_context(void)
{
    return false;
}

bool
PmatchObject::is_delimiter(void)
{
    return false;
}

bool
PmatchUnaryOperation::is_context(void)
{
    return op == LC || op == NLC || op == RC || op == NRC;
}

bool
PmatchUnaryOperation::is_delimiter(void)
{
    return op == AddDelimiters;
}

StringSet
PmatchObject::get_initial_RC_initial_symbols(void)
{
    return StringSet();
}

StringSet
PmatchObject::get_initial_NRC_initial_symbols(void)
{
    return StringSet();
}

StringSet
PmatchBinaryOperation::get_initial_RC_initial_symbols(void)
{
    StringSet retval;
    if (op == Concatenate)
    {
...
StringSet
PmatchBinaryOperation::get_initial_NRC_initial_symbols(void)
{
    StringSet retval;
    if (op == Concatenate)
    {
...
StringSet
PmatchUnaryOperation::get_initial_RC_initial_symbols(void)
{
    if (op == RC)
    {
...
StringSet
PmatchUnaryOperation::get_initial_NRC_initial_symbols(void)
{
    if (op == NRC)
    {
        HfstTransducer *tmp = this->root->evaluate();
...
void
PmatchObject::collect_initial_symbols_into(
    StringSet &allowed_initial_symbols, StringSet &disallowed_initial_symbols)
{
    ...
    if (string_set_has_meta_arc(allowed))
    {
        if (required.size() != 0 && !string_set_has_meta_arc(required))
        {
            ...
            for (StringSet::iterator it = required.begin();
                 it != required.end(); ++it)
            {
        ...
        else
        {
            ...
            {
                disallowed_initial_symbols.insert(disallowed.begin(),
                                                  disallowed.end());
                return;
            }
        }
    ...
    if (required.size() == 0 || string_set_has_meta_arc(required))
    {
        ...
        for (StringSet::iterator it = allowed.begin(); it != allowed.end();
             ++it)
        {
            if (disallowed.count(*it) == 0)
            {
...
void
PmatchString::collect_strings_into(StringVector &strings)
{
    strings.push_back(string);
}

void
PmatchBinaryOperation::collect_strings_into(StringVector &strings)
{
...
HfstTransducer *
PmatchSymbol::evaluate(void)
{
    if (name != "")
    {
        eval_stack.push_back(name);
    }
    start_timing();
    HfstTransducer *retval = NULL;
    if (symbol_in_local_context(sym))
...
PmatchObject *
PmatchSymbol::evaluate_as_arg(void)
{
    if (symbol_in_local_context(sym))
    {
...
void
PmatchSymbol::collect_strings_into(StringVector &strings)
{
    if (symbol_in_local_context(sym))
    {
...
PmatchObject *
PmatchString::evaluate_as_arg(void)
{
    return new PmatchString(*this);
}

HfstTransducer *
PmatchFunction::evaluate(std::vector<PmatchObject *> funargs)
{
    if (verbose)
    {
        my_timer = clock();
    ...
    {
        std::stringstream errstring;
        errstring << "Function " << name << " expected " << args.size()
                  << " args, got " << funargs.size() << std::endl;
        throw std::invalid_argument(errstring.str());
    }
    ...
    if (verbose)
    {
        double duration = (clock() - my_timer) / (double)CLOCKS_PER_SEC;
        write_compilation_stack_indentation_to_err();
        std::cerr << "Call to " << name << " evaluated in " << duration
                  << " seconds\n";
        --named_object_evaluation_stack_depth;
    }
    return retval;
...
PmatchFuncall::evaluate(void)
{
    ...
    for (std::vector<PmatchObject *>::iterator it = args->begin();
         it != args->end(); ++it)
    {
        evaluated_args.push_back((*it)->evaluate_as_arg());
    }
    HfstTransducer *retval = fun->evaluate(evaluated_args);
    for (std::vector<PmatchObject *>::iterator it = evaluated_args.begin();
         it != evaluated_args.end(); ++it)
    {
        delete *it;
    }
    if (name != "")
...
HfstTransducer *
PmatchBuiltinFunction::evaluate(void)
{
    if (name != "")
    {
        eval_stack.push_back(name);
    ...
    if (type == Interpolate)
    {
        ...
        {
            std::stringstream errstring;
            errstring << "Builtin function Interpolate called with "
                      << args->size()
                      << " arguments, but it expects at least 3." << std::endl;
            throw std::invalid_argument(errstring.str());
        }
        ...
        for (std::vector<PmatchObject *>::reverse_iterator it
             = args->rbegin() + 2;
             it != args->rend(); ++it)
        {
...
HfstTransducer *
PmatchNumericOperation::evaluate(void)
{
    if (cache != NULL)
    {
        report_cache();
...
HfstTransducer *
PmatchUnaryOperation::evaluate(void)
{
    if (cache != NULL)
    {
        report_cache();
    ...
    else if (op == Complement)
    {
        ...
        HfstTransducer *complement = new HfstTransducer(
            hfst::internal_identity, hfst::pmatch::format);
    ...
    else if (op == TermComplement)
    {
        ...
        for (hfst::StringSet::iterator it = alphabet.begin();
             it != alphabet.end(); ++it)
        {
    ...
    else if (op == LC)
    {
        ...
        {
            retval->reverse();
            HfstTransducer *tmp = new HfstTransducer(hfst::internal_epsilon,
                                                     LC_ENTRY_SYMBOL, format);
            tmp->concatenate(*retval);
            HfstTransducer lc_exit(hfst::internal_epsilon, LC_EXIT_SYMBOL,
                                   format);
        ...
        {
            write_compilation_stack_indentation_to_err();
            std::cerr << "** Warning: ignoring nested context condition when "
                         "compiling "
                      << eval_stack.back() << std::endl;
        }
    }
    else if (op == NLC)
    {
        if (!transducer_has_context_symbol(retval))
        {
            ...
            HfstTransducer *head = tmp->evaluate();
            delete tmp;
            HfstTransducer passthrough(PASSTHROUGH_SYMBOL, format);
            HfstTransducer nlc_entry(hfst::internal_epsilon, NLC_ENTRY_SYMBOL,
                                     format);
            HfstTransducer nlc_exit(hfst::internal_epsilon, NLC_EXIT_SYMBOL,
                                    format);
    ...
    else if (op == RC)
    {
        ...
        {
            HfstTransducer *tmp = new HfstTransducer(hfst::internal_epsilon,
                                                     RC_ENTRY_SYMBOL, format);
            tmp->concatenate(*retval);
            HfstTransducer rc_exit(hfst::internal_epsilon, RC_EXIT_SYMBOL,
                                   format);
    ...
    else if (op == NRC)
    {
        if (!transducer_has_context_symbol(retval))
        {
            ...
            delete tmp;
            HfstTransducer passthrough(PASSTHROUGH_SYMBOL, format);
            HfstTransducer nrc_entry(hfst::internal_epsilon, NRC_ENTRY_SYMBOL,
                                     format);
            HfstTransducer nrc_exit(hfst::internal_epsilon, NRC_EXIT_SYMBOL,
                                    format);
...
PmatchBinaryOperation::evaluate(void)
{
    ...
    {
        HfstTransducer *tmp
            = new HfstTransducer(hfst::xeroxRules::before(*lhs, *rhs));
        delete lhs;
        lhs = tmp;
    }
    else if (op == After)
    {
        HfstTransducer *tmp
            = new HfstTransducer(hfst::xeroxRules::after(*lhs, *rhs));
        delete lhs;
        lhs = tmp;
    }
...
StringPair
PmatchBinaryOperation::as_string_pair(void)
{
    if (op == CrossProduct)
    {
...
bool
PmatchBinaryOperation::is_unweighted_disjunction_of_strings(void)
{
    return weight == 0.0 && op == Disjunct
           && left->is_unweighted_disjunction_of_strings()
           && right->is_unweighted_disjunction_of_strings();
}

HfstTransducer *
PmatchTernaryOperation::evaluate(void)
{
    if (cache != NULL)
    {
        report_cache();
...
PmatchAcceptor::evaluate(void)
{
    ...
    case UppercaseAlpha:
        ...
        {
            retval
                = new HfstTransducer(*get_utils()->latin1_uppercase_acceptor);
        }
        break;
    case LowercaseAlpha:
        if (variables["unicode-character-classes"] == "on")
        ...
        {
            retval
                = new HfstTransducer(*get_utils()->latin1_lowercase_acceptor);
        }
        break;
    case Numeral:
    ...
    case Whitespace:
        ...
        {
            retval
                = new HfstTransducer(*get_utils()->latin1_whitespace_acceptor);
        }
    }
    retval->set_final_weights(hfst::double_to_float(weight), true);
    report_time();
...
bool
transducer_has_context_symbol(HfstTransducer *t)
{
    StringSet ss = t->get_alphabet();
    return ss.count(LC_ENTRY_SYMBOL) == 1 || ss.count(NLC_ENTRY_SYMBOL) == 1
           || ss.count(RC_ENTRY_SYMBOL) == 1
           || ss.count(NRC_ENTRY_SYMBOL) == 1;
}

void
warn_on_nonsubtractable_symbols(HfstTransducer *t)
{
    StringSet alphabet = t->get_alphabet();
    for (StringSet::const_iterator it = alphabet.begin(); it != alphabet.end();
         ++it)
    {
        if (it->size() < 3)
        {
        ...
        else if (it->find("@PMATCH") == 0 || it->find("@I") == 0
                 || it->find("@L") == 0)
        {
            write_compilation_stack_indentation_to_err();
            std::cerr << "Warning: subtracting with nonsubtractable symbol "
                      << *it << std::endl;
        }
    }
}
...
HfstTransducer *
PmatchParallelRulesContainer::evaluate(void)
{
    if (cache != NULL)
    {
        report_cache();
    ...
    case hfst::xeroxRules::E_RTL_LONGEST_MATCH:
        retval = new HfstTransducer(
            replace_rightmost_longest_match(make_mappings()));
        break;
    case hfst::xeroxRules::E_RTL_SHORTEST_MATCH:
        retval = new HfstTransducer(
            replace_rightmost_shortest_match(make_mappings()));
        break;
    case hfst::xeroxRules::E_LTR_LONGEST_MATCH:
        retval = new HfstTransducer(
            replace_leftmost_longest_match(make_mappings()));
        break;
    case hfst::xeroxRules::E_LTR_SHORTEST_MATCH:
        retval = new HfstTransducer(
            replace_leftmost_shortest_match(make_mappings()));
        break;
...
PmatchParallelRulesContainer::make_mappings(void)
{
    std::vector<hfst::xeroxRules::Rule> retval;
    for (std::vector<PmatchReplaceRuleContainer *>::iterator it
         = rules.begin();
         it != rules.end(); ++it)
    {
...
HfstTransducer *
PmatchReplaceRuleContainer::evaluate(void)
{
    if (cache != NULL)
    {
        report_cache();
    ...
    case hfst::xeroxRules::E_RTL_LONGEST_MATCH:
        retval = new HfstTransducer(
            replace_rightmost_longest_match(make_mapping()));
        break;
    case hfst::xeroxRules::E_RTL_SHORTEST_MATCH:
        retval = new HfstTransducer(
            replace_rightmost_shortest_match(make_mapping()));
        break;
    case hfst::xeroxRules::E_LTR_LONGEST_MATCH:
        retval = new HfstTransducer(
            replace_leftmost_longest_match(make_mapping()));
        break;
    case hfst::xeroxRules::E_LTR_SHORTEST_MATCH:
        retval = new HfstTransducer(
            replace_leftmost_shortest_match(make_mapping()));
        break;
    case hfst::xeroxRules::E_REPLACE_RIGHT_MARKUP:
    default:
...
PmatchReplaceRuleContainer::make_mapping(void)
{
    HfstTransducerPairVector pair_vector;
    for (MappingPairVector::iterator it = mapping.begin(); it != mapping.end();
         ++it)
    {
        TransducerPointerPair pp = (*it)->evaluate_pair();
        HfstTransducerPair p(HfstTransducer(*(pp.first)),
                             HfstTransducer(*(pp.second)));
        delete pp.first;
        delete pp.second;
    ...
    for (MappingPairVector::iterator it = context.begin(); it != context.end();
         ++it)
    {
        TransducerPointerPair pp = (*it)->evaluate_pair();
        HfstTransducerPair p(HfstTransducer(*(pp.first)),
...
HfstTransducer *
PmatchQuestionMark::evaluate(void)
{
    start_timing();
    HfstTransducer *retval = NULL;
...
HfstTransducer *
PmatchRestrictionContainer::evaluate(void)
{
    if (cache != NULL)
    {
        report_cache();
    ...
    retval
        = new HfstTransducer(hfst::xeroxRules::restriction(*l, pair_vector));
    delete l;
    retval->set_final_weights(hfst::double_to_float(weight), true);
    report_time();
    if (cache == NULL && should_use_cache() == true)
...
PmatchMarkupContainer::evaluate_pair(void)
{
    ...
    HfstTransducerPair MappingPair
        = hfst::xeroxRules::create_mapping_for_mark_up_replace(tmpMappingPair,
                                                               marks);
    delete loa;
    delete lom;
    delete rom;
...
HfstTransducer *
PmatchMappingPairsContainer::evaluate(void)
{
    pmatcherror("Should never happen\n");
    return 0;
}
HfstTransducer *
PmatchContextsContainer::evaluate(void)
{
    pmatcherror("Should never happen\n");
    return 0;
}
