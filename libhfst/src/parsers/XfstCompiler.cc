...
#define GET_TOP(x)                                                            \
    HfstTransducer *x = this->top();                                          \
    if ((x) == NULL)                                                          \
    {                                                                         \
        xfst_lesser_fail();                                                   \
        return *this;                                                         \
    }
#define PROMPT_AND_RETURN_THIS                                                \
    prompt();                                                                 \
    return *this;
#define PRINT_INFO_PROMPT_AND_RETURN_THIS                                     \
    print_transducer_info();                                                  \
    prompt();                                                                 \
    return *this;
#define IF_NULL_PROMPT_AND_RETURN_THIS(x)                                     \
    if (x == NULL)                                                            \
    {                                                                         \
...
#define MAYBE_ASSERT(assertion, value)                                        \
    if (!value                                                                \
        && ((variables_["assert"] == "ON" || assertion)                       \
...
#define CHECK_FILENAME(x)                                                     \
    if (!this->check_filename(x))                                             \
    {                                                                         \
        return *this;                                                         \
    }
#define EMPTY_STACK                                                           \
    error() << "Empty stack." << std::endl;                                   \
    flush(&error());
...
{
    variable_explanations_["assert"]
        = "quit the application if test result is 0 and quit-on-fail is ON";
    variable_explanations_["att-epsilon"]
    ...
    variable_explanations_["encode-weights"]
        = "encode weights when minimizing";
    variable_explanations_["flag-is-epsilon"]
        = "treat flag diacritics as epsilons in composition";
    variable_explanations_["harmonize-flags"]
        = "harmonize flag diacritics before composition";
    variable_explanations_["hopcroft-min"]
        = "use hopcroft's minimization algorithm";
    variable_explanations_["lexc-minimize-flags"]
        = "if 'lexc-with-flags' == ON, minimize number of flags";
    variable_explanations_["lexc-rename-flags"]
        = "if 'lexc-minimize-flags' == ON, rename flags";
    variable_explanations_["lexc-with-flags"]
        = "use flags to hyperminimize result from lexc files";
    variable_explanations_["maximum-weight"]
        = "maximum weight of paths printed in apply";
    variable_explanations_["minimal"] = "minimize networks after operations";
    variable_explanations_["name-nets"]
        = "stores the name of the network when using 'define'";
    variable_explanations_["obey-flags"] = "obey flag diacritic constraints";
    variable_explanations_["precision"]
        = "todo: precision to use when printing weights";
    variable_explanations_["print-foma-sigma"] = "print identities as '@'";
    variable_explanations_["print-pairs"]
        = "show both sides (upper and lower) of labels";
    variable_explanations_["print-sigma"]
        = "show sigma when printing a network";
    variable_explanations_["print-space"]
        = "insert a space between symbols when printing words";
    variable_explanations_["print-weight"]
        = "show weights when printing words or networks";
    variable_explanations_["quit-on-fail"]
        = "quit the application if a command cannot be executed";
    variable_explanations_["quote-special"]
    ...
    variable_explanations_["retokenize"]
        = "retokenize regular expressions in 'compile-replace'";
    variable_explanations_["show-flags"]
    ...
    variable_explanations_["xerox-composition"]
...
XfstCompiler::XfstCompiler()
    : use_readline_(false), read_interactive_text_from_stdin_(false),
      output_to_console_(false), xre_(hfst::TROPICAL_OPENFST_TYPE),
      lexc_(hfst::TROPICAL_OPENFST_TYPE), format_(hfst::TROPICAL_OPENFST_TYPE),
      verbose_(false), verbose_prompt_(false), latest_regex_compiled(NULL),
...
XfstCompiler::XfstCompiler(ImplementationType impl)
    : use_readline_(false), read_interactive_text_from_stdin_(false),
      output_to_console_(false), xre_(impl), lexc_(impl), format_(impl),
      verbose_(false), verbose_prompt_(false), latest_regex_compiled(NULL),
      quit_requested_(false), fail_flag_(false), output_(&std::cout),
      error_(&std::cerr),
#ifdef WINDOWS
...
int
XfstCompiler::xfst_fclose(FILE *f, const char *name)
{
    int retval = fclose(f);
    if (retval != 0)
    {
...
FILE *
XfstCompiler::xfst_fopen(const char *path, const char *mode)
{
...
void
XfstCompiler::set_error_stream(std::ostream &os)
{
    error_ = &os;
...
std::ostream &
XfstCompiler::get_error_stream()
{
    return *error_;
}

void
XfstCompiler::set_output_stream(std::ostream &os)
{
    output_ = &os;
}

std::ostream &
XfstCompiler::get_output_stream()
{
    return *output_;
}

int
XfstCompiler::get_precision()
{
...
void
XfstCompiler::xfst_lesser_fail()
{
    if (variables_["quit-on-fail"] == "ON"
        && !read_interactive_text_from_stdin_)
    {
        this->fail_flag_ = true;
    }
}
...
extract_output_paths(const HfstTwoLevelPaths &paths)
{
    ...
    for (const auto &it : paths)
    {
        ...
        {
            if (p.second != "@0@" && p.second != "@_EPSILON_SYMBOL_@")
            {
                          ...
                          std::ostream *oss_ /* =cout */, int n /* = -1*/)
{
         ...
         n != 0 && it != paths.end(); it++)
    {
        ...
        {
            StringVector path_input
                = hfst::symbols::to_string_vector(path, true /*input side*/);
...
static float
string_to_float(const std::string &str)
{
...
XfstCompiler::lookup(char *line, HfstBasicTransducer *t)
{
                                          ...
                                          variables_["obey-flags"] == "ON"))
    {
        ...
        if (verbose_)
        {
            error() << "warning: lookup is infinitely ambiguous, limiting the "
                       "number of cycles to "
                    << cutoff << std::endl;
            flush(&error());
        }
    ...
    if (variables_["maximum-weight"] == "OFF")
        t->lookup(lookup_path, results, &cutoff, NULL, -1 /*max_number*/,
                  (variables_["obey-flags"] == "ON"));
    else
    {
        float max_weight = string_to_float(variables_["maximum-weight"]);
        t->lookup(lookup_path, results, &cutoff, &max_weight,
                  -1 /*max_number*/, (variables_["obey-flags"] == "ON"));
    }
...
XfstCompiler::lookup_optimize()
{
    ...
    if (verbose_)
    {
        error() << "converting transducer type from "
                << hfst::implementation_type_to_format(t->get_type()) << " to "
...
XfstCompiler::remove_optimization()
{
    ...
    if (verbose_)
    {
        ...
        {
            error() << "warning: converting from weighted to unweighted "
                       "format, loss of information is possible"
                    << std::endl;
            flush(&error());
        }
    }
...
XfstCompiler::apply_down_line(char *line) // apply_up_line -> apply_down_line
{
    ...
    if (t->get_type() != hfst::HFST_OL_TYPE
        && t->get_type() != hfst::HFST_OLW_TYPE)
    {
        // hfst_fprintf(warnstream_, "lookup might be slow, consider 'convert
        // net'\n");
    ...
    size_t ol_cutoff
        = string_to_size_t(variables_["lookup-cycle-cutoff"]); // -1; fix this
    StringVector foo; // this gets ignored by ol transducer's
    ...
    if (t->is_lookup_infinitely_ambiguous(foo))
    {
        ...
        if (verbose_)
        {
            error() << "warning: transducer is infinitely ambiguous, limiting "
                       "number of cycles to "
                    << ol_cutoff << std::endl;
            flush(&error());
        }
...
XfstCompiler::apply_up_line(char *line) // apply_down_line -> apply_up_line
{
    ...
    if (verbose_)
    {
        error() << "warning: apply up not implemented, inverting transducer "
                   "and performing apply down"
                << std::endl
                << "for faster performance, invert and minimize top network "
                   "and do apply down instead"
                << std::endl
                << std::endl;
        flush(&error());
    }
    ...
    t->invert()
        .minimize(); // the user has been warned for possible slow performance
    HfstBasicTransducer fsm(*t);
...
int
XfstCompiler::unknown_command(const char *s)
{
...
bool
XfstCompiler::get_fail_flag() const
{
    return this->fail_flag_;
}

bool
XfstCompiler::quit_requested() const
{
...
XfstCompiler::apply(FILE *infile, ApplyDirection direction)
{
    ...
    HfstTransducer *t = stack_.top();
    size_t ol_cutoff = string_to_size_t(variables_["lookup-cycle-cutoff"]);
    ; // -1; fix this // number of cycles needs to be limited for an infinitely
      // ambiguous ol transducer
    ...
    if (direction == APPLY_UP_DIRECTION)
    {
        if (t->get_type() == hfst::HFST_OL_TYPE
            || t->get_type() == hfst::HFST_OLW_TYPE)
        {
            error() << "Operation not supported for optimized lookup format. "
                       "Consider 'remove-optimization' to convert into "
                       "ordinary format."
                    << std::endl;
            flush(&error());
            prompt();
        ...
        if (verbose_)
        {
            error() << "warning: apply up not implemented, inverting "
                       "transducer and performing apply down"
                    << std::endl
                    << "for faster performance, invert and minimize top "
                       "network and do apply down instead"
                    << std::endl
                    << std::endl;
            flush(&error());
        }
        t = new HfstTransducer(*(stack_.top()));
        t->invert().minimize(); // the user has been warned for possible slow
                                // performance
    }
    ...
    else
    {
        ...
        if (t->is_lookup_infinitely_ambiguous(foo))
        {
            ...
            if (verbose_)
            {
                error() << "warning: transducer is infinitely ambiguous, "
                           "limiting number of cycles to "
                        << ol_cutoff << std::endl;
                flush(&error());
            }
        }
...
XfstCompiler::define_list(const char *name, const char *start, const char *end)
{
    ...
    {
        error() << "unsupported unicode range " << std::string(start) << "-"
...
XfstCompiler::define_list(const char *name, const char *values)
{
    ...
    {
        error() << "Error: '" << std::string(name)
                << "' has already been defined as a transducer variable."
                << std::endl
                << "It cannot have an incompatible definition as a list."
                << std::endl
                << "Please undefine the definition first." << std::endl;
        flush(&error());
        // fprintf(warnstream_, "Error: '%s' has already been defined as a
        // transducer variable.\n"
        //         "It cannot have an incompatible definition as a list.\n"
...
XfstCompiler::define(const char *name, const char *xre)
{
    /*if (hfst::xfst::nametoken_to_number(name) >= 0)
      {
        hfst_fprintf(errorstream_, "Could not define variable, '%s' is not a
      valid name\n", name);
                }*/
    ...
    {
        error() << "Error: '" << std::string(name)
                << "' has already been defined as a list variable."
                << std::endl
...
static bool
extract_function_name(const char *prototype, std::string &name)
{
...
// \a prototype must be of format "functionname(arg1, arg2, ... argN)"
static bool
extract_function_arguments(const char *prototype,
...
/* Convert each argument name in \a arguments in \a xre with a special symbol
   ...
   For example, if we have a function Foo(x, y, z) defined with regex [ x y+ z
   x* ], the regex is converted into ["@Foo(1@" "@Foo(2@"+ "@Foo(3@" "@Foo(1@"*
   ].
*/
std::string
convert_argument_symbols(const std::vector<std::string> &arguments,
                         ...
                         bool user_friendly_argument_names = false)
{
    ...
    for (const auto &argument : arguments)
    {
        ...
        if (!xre_.get_positions_of_symbol_in_xre(argument, retval,
                                                 arg_positions)) // XRE
        {
            return std::string("");
        }
        ...
        for (unsigned int i = 0; i < retval.length(); i++)
        {
            ...
            if (arg_positions.find(i) != arg_positions.end())
            {
                ...
                for (unsigned int offset = 1; offset < argument.length();
                     offset++)
                {
                    ++i;
                }
...
XfstCompiler::define_function(const char *prototype, const char *xre)
{
    ...
    {
        error() << "Error extracting function name from prototype '"
                << std::string(prototype) << "'" << std::endl;
        flush(&error());
        xfst_fail();
        PROMPT_AND_RETURN_THIS;
    }
    ...
    {
        error() << "Error extracting function arguments from prototype '"
                << std::string(prototype) << "'" << std::endl;
        flush(&error());
        xfst_fail();
        PROMPT_AND_RETURN_THIS;
    ...
    if (!xre_.define_function(name, hfst::size_t_to_uint(arguments.size()),
                              xre_converted)) // XRE
    {
    ...
    if (verbose_)
    {
        ...
        output() << " function '" << name << "@" << (int)arguments.size()
                 << ")" << std::endl;
        flush(&output());
    }
...
static std::string
to_upper_case(const std::string &str)
{
    std::string retval;
...
static bool
string_found(const std::string &str_, const std::string &text_)
{
...
{
    int help_mode = (strcmp(text, "") == 0) ? HELP_MODE_ALL_COMMANDS
...
XfstCompiler::add_loaded_definition(HfstTransducer *t)
{
    ...
    {
        error()
            << "warning: loaded transducer definition has no name, skipping it"
            << std::endl;
        flush(&error());
        return *this;
    }
    ...
    if (it != definitions_.end())
    {
        error() << "warning: a definition named '" << def_name
...
void
XfstCompiler::convert_to_common_format(HfstTransducer *t,
                                       const char *filename /*=NULL*/)
{
    ...
    if (t->get_type() != format_)
    {
            ...
            || t->get_type() == hfst::HFST_OLW_TYPE)
        {
            ...
            {
                error()
                    << "warning: transducer is in optimized lookup format, "
                       "'apply up' is the only operation it supports"
                    << std::endl;
                flush(&error());
            }
            return;
        ...
        if (verbose_)
        {
            error() << "warning: converting transducer type from "
                    << hfst::implementation_type_to_format(t->get_type())
                    << " to " << hfst::implementation_type_to_format(format_);
            flush(&error());
            if (filename != NULL)
            {
            ...
            if (!hfst::HfstTransducer::is_safe_conversion(t->get_type(),
                                                          format_))
            {
...
XfstCompiler::open_hfst_input_stream(const char *infilename)
{
    ...
    if (!this->check_filename(infilename))
    {
        return NULL;
    }
    ...
    try
    {
        instream = (infilename != 0) ? new HfstInputStream(infilename)
                                     : new HfstInputStream();
    }
    catch (NotTransducerStreamException ntse)
    {
        error() << "Unable to read transducers from "
                << to_filename(infilename) << std::endl;
        flush(&error());
        xfst_fail();
        return NULL;
    }
...
XfstCompiler &
XfstCompiler::load_stack_or_definitions(const char *infilename,
                                        bool load_definitions)
{
    CHECK_FILENAME(infilename);
    // Try to open the stream to file infilename
    std::unique_ptr<HfstInputStream> instream{ open_hfst_input_stream(
        infilename) };
    ...
    while (instream->is_good())
    {
        ...
        if (load_definitions)
        {
            if (t->get_type() == hfst::HFST_OL_TYPE
                || t->get_type() == hfst::HFST_OLW_TYPE)
            {
                error() << "cannot load optimized lookup transducers as "
                           "definitions"
                        << std::endl;
                flush(&error());
                break;
...
XfstCompiler::eliminate_flag(const char *name)
{
    ...
    {
        error() << "error: could not eliminate flag '" << name
                << "': " << e.name << std::endl;
        flush(&error());
        MAYBE_QUIT;
    }
...
XfstCompiler::system(const char *command)
{
    if (restricted_mode_)
    {
        error() << "Restricted mode (--restricted-mode) is in use, system "
                   "calls are disabled"
                << std::endl;
        flush(&error());
        xfst_lesser_fail();
...
XfstCompiler::set(const char *name, const char *text)
{
    if (variables_.find(name) == variables_.end())
    {
        ...
        {
            error() << "variable compose-flag-as-special not found, using "
                       "flag-is-epsilon instead"
                    << std::endl;
            flush(&error());
...
XfstCompiler::show()
{
    for (const auto &var : variables_)
    {
        ...
        else
        {
            ...
            output() << var.first << ": ";
            output().width(6);
            output() << var.second << ": " << variable_explanations_[var.first]
                     << std::endl;
        }
    }
    flush(&output());
        ...
        || retval->get_type() == hfst::HFST_OLW_TYPE)
    {
        error()
            << "Operation not supported for optimized lookup format. Consider "
               "'remove-optimization' to convert into ordinary format."
            << std::endl;
        flush(&error());
        prompt();
        return NULL;
...
XfstCompiler::test_uni(Level level, bool assertion)
{
    ...
    {
        error()
            << "ERROR: argument given to function 'test_uni' not recognized"
            << std::endl;
        flush(&error());
    }
...
XfstCompiler::test_operation(TestOperation operation, bool assertion)
{
    ...
    {
        error()
            << "Not enough networks on stack. Operation requires at least 2."
            << std::endl;
        flush(&error());
        xfst_lesser_fail();
        PROMPT_AND_RETURN_THIS;
...
static StringVector
tokenize_string(const char *s, char c)
{
    StringVector retval;
    std::string str(s);
    size_t pos = 0;
...
static StringPair
symbol_vector_to_symbol_pair(const StringVector &sv)
{
    StringPair sp;
    if (sv.size() == 2)
    {
...
XfstCompiler::substitute_named(const char *variable, const char *label)
{
    ...
    {
        error() << "no occurrences of label '" << label
                << "', cannot substitute" << std::endl;
        flush(&error());
        MAYBE_QUIT;
        PROMPT_AND_RETURN_THIS;
    }
    ...
    for (const auto &it : fsm)
    {
        for (const auto &tr_it : it)
        {
            ...
            {
                error() << "label '" << label
                        << "' is used as a symbol on one side of an arc, "
                           "cannot substitute"
                        << std::endl;
                flush(&error());
                MAYBE_QUIT;
...
XfstCompiler::substitute_label(const char *list, const char *target)
{
    ...
    try
    {
        ...
        {
            error() << "no occurrences of '" << target_label.first << ":"
                    << target_label.second << "', cannot substitute"
                    << std::endl;
            flush(&error());
            PROMPT_AND_RETURN_THIS;
        }
...
XfstCompiler::substitute_symbol(const char *list, const char *target)
{
    ...
    {
        error() << "no occurrences of symbol '" << target
                << "', cannot substitute" << std::endl;
        flush(&error());
        MAYBE_QUIT;
        PROMPT_AND_RETURN_THIS;
    ...
    if (liststr
        == "\"NOTHING\"") // list is given in quoted format: "foo" "bar" ...
        liststr = "";
...
XfstCompiler::print_label_count(std::ostream *oss_)
{
    ...
    for (const auto &it : fsm)
    {
        for (const auto &tr_it : it)
        {
            label_map[{ tr_it.get_input_symbol(),
...
XfstCompiler &
XfstCompiler::shortest_string(const hfst::HfstTransducer *transducer,
...
XfstCompiler &
XfstCompiler::print_one_string_or_its_size(std::ostream *oss_,
                                           const HfstTwoLevelPaths &paths,
                                           const char *level, bool print_size)
{
    std::ostream *oss = get_stream(oss_);
    assert(level != NULL);
...
XfstCompiler &
XfstCompiler::print_longest_string_or_its_size(std::ostream *oss_,
                                               bool print_size)
{
    std::ostream *oss = get_stream(oss_);
    GET_TOP(topmost);
    ...
    { // Transducer is empty if neither upper..
        transducer_is_empty = !tmp_upper.extract_longest_paths(
            paths_upper, variables_["obey-flags"] == "ON");
    }
    ...
    { // ..nor lower paths can be extracted.
        transducer_is_empty = !tmp_lower.extract_longest_paths(
            paths_lower, variables_["obey-flags"] == "ON");
    }
    catch (const TransducerIsCyclicException &e)
    {
        (void)e;
    ...
    {
        output() << "transducer is cyclic" << std::endl;
        flush(&output());
    }
    ...
    else
    {
        ...
        {
            error() << "warning: longest string may have flag diacritics that "
                       "are not shown"
                    << std::endl;
            error() << "         but are used in calculating its length (use "
                       "'eliminate flags')"
                    << std::endl;
            flush(&error());
        }
        ...
        {
            print_one_string_or_its_size(oss, paths_upper, "Upper",
                                         print_size);
        }
        ...
        {
            print_one_string_or_its_size(oss, paths_lower, "Lower",
                                         print_size);
        }
    }
...
XfstCompiler &
XfstCompiler::print_random_lower(const char *name, unsigned int number,
                                 std::ostream *oss_)
{
    std::ostream *oss = get_stream(oss_);
    hfst::HfstTwoLevelPaths paths;
    ...
    else
    {
        ...
        if (it == definitions_.end())
        {
            *oss << "no such definition '" << std::string(name) << "'"
                 << std::endl;
            flush(oss);
            prompt();
            return *this;
...
XfstCompiler &
XfstCompiler::print_random_upper(const char *name, unsigned int number,
                                 std::ostream *oss_)
{
    std::ostream *oss = get_stream(oss_);
    hfst::HfstTwoLevelPaths paths;
                          ...
                          std::ostream *oss_, Level level)
{
    ...
    default:
        error()
            << "ERROR: argument given to function 'print_words' not recognized"
            << std::endl;
        flush(&error());
        flush(oss);
        PROMPT_AND_RETURN_THIS;
    ...
    {
        (void)e;
        int cutoff = hfst::size_t_to_uint(
            string_to_size_t(variables_["print-words-cycle-cutoff"]));
        error() << "warning: transducer is cyclic, limiting the number of "
                   "cycles to "
                << cutoff << std::endl;
        flush(&error());
        if (variables_["obey-flags"] == "OFF")
...
XfstCompiler &
XfstCompiler::print_random_words(const char *name, unsigned int number,
                                 std::ostream *oss_)
{
    std::ostream *oss = get_stream(oss_);
...
std::ostream *
XfstCompiler::get_stream(std::ostream *oss)
{
#ifdef WINDOWS
...
void
XfstCompiler::flush(std::ostream *oss)
{
#ifdef WINDOWS
    if (output_to_console_)
    {
...
XfstCompiler::view_net()
{
    ...
    if (false || verbose_)
    {
        error() << "Writing net in dot format to temporary file '"
                << dotfilename << "'." << std::endl;
        flush(&error());
    }
    ...
    if (false || verbose_)
    {
        error() << "Wrote net, closing file and converting into png format."
                << std::endl;
        flush(&error());
    }
    if (::system(("dot -Tpng " + std::string(dotfilename) + " > "
                  + std::string(pngfilename) + " 2> /dev/null")
                     .c_str())
        != 0)
    {
    ...
    if (::system(("/usr/bin/xdg-open " + std::string(pngfilename)
                  + " 2> /dev/null &")
                     .c_str())
        != 0)
    {
...
static bool
is_special_symbol(const std::string &s)
{
...
void
XfstCompiler::print_alphabet(const StringSet &alpha, bool unknown,
                             bool identity, std::ostream *oss_)
{
    std::ostream *oss = get_stream(oss_);
...
static bool
is_unknown_or_identity_used_in_transducer(HfstTransducer *t, bool &unknown,
                                          bool &identity)
{
    unknown = false;
    identity = false;
...
XfstCompiler::write_prolog(std::ostream *oss_)
{
    ...
    while (stack_.size() != 0)
    {
        ...
        if (name.empty())
            name = "NO_NAME";
        HfstBasicTransducer fsm(*tr);
        fsm.write_in_prolog_format(*oss, name,
                                   variables_["print-weight"] == "ON");
...
XfstCompiler::check_filename(const char *filename)
{
    if (restricted_mode_)
    {
        ...
        {
            error() << "Restricted mode (--restricted-mode) is in use, write "
                       "and read operations are allowed"
                    << std::endl
                    << "only in current directory (i.e. filenames cannot "
                       "contain '/' or '\\')"
                    << std::endl;
            flush(&error());
            xfst_lesser_fail();
...
XfstCompiler::write_stack(const char *filename)
{
    ...
    auto outstream
        = (filename != 0)
              ? std::make_unique<HfstOutputStream>(filename,
                                                   stack_.top()->get_type())
...
#define MAX_FILE_SIZE 10000000
    ...
    size_t read = 0;
    char *file_data
        = static_cast<char *>(malloc(sizeof(char) * MAX_FILE_SIZE + 1));
...
XfstCompiler::read_text_or_spaced(const char *filename, bool spaces)
{
    ...
    while ((line = xfst_getline(infile)) != NULL)
    {
        ...
        StringPairVector spv
            = tok.tokenize_pair_string(std::string(line), spaces);
...
XfstCompiler::apply_unary_operation(UnaryOperation operation)
{
    ...
    try
    {
        ...
        case REVERSE_NET:
            ...
            break;
        case MINIMIZE_NET:
            result->minimize(); // implicit minimization requested, do not use
                                // optimize()
            break;
        case PRUNE_NET_:
            result->prune();
        ...
        if (operation != MINIMIZE_NET && operation != DETERMINIZE_NET
            && operation != EPSILON_REMOVE_NET)
        {
            MAYBE_MINIMIZE(result);
        }
        stack_.push(result);
        ...
        case COMPOSE_NET:
        {
            if (result->has_flag_diacritics() && t->has_flag_diacritics())
            {
                if (variables_["harmonize-flags"] == "OFF")
                {
                    if (verbose_)
                    {
                        error() << "Both composition arguments contain flag "
            ...
            {
                (void)e;
                error() << "Error: flag diacritics must be identities in "
...
XfstCompiler::negate_net()
{
    ...
    catch (const TransducerIsNotAutomatonException &e)
    {
        ...
        error() << "Error: Negation is defined only for automata." << std::endl
                << "Use expression [[?:?]* - A] instead where A is the "
                   "transducer to be negated."
                << std::endl;
        flush(&error());
        xfst_lesser_fail();
        return *this;
...
void
XfstCompiler::print_level(const std::vector<unsigned int> &whole_path,
...
static void
append_state_to_paths(std::vector<unsigned int> &whole_path,
...
static bool
return_to_level(std::vector<unsigned int> &whole_path,
                std::vector<unsigned int> &shortest_path, unsigned int level)
{
...
char *
XfstCompiler::remove_newline(char *line)
{
    unsigned int i = 0;
    while (line[i] != '\0')
    {
...
char *
XfstCompiler::xfst_getline(FILE *file, const std::string &promptstr)
{
#ifdef HAVE_READLINE
...
int
XfstCompiler::current_history_index()
{
#ifdef HAVE_READLINE
...
void
XfstCompiler::ignore_history_after_index(int index)
{
#ifdef HAVE_READLINE
    for (unsigned int i = (history_length - 1); i > (index - 1); i--)
    {
        remove_history(i);
    }
#endif
...
bool
XfstCompiler::can_arc_be_followed(int number, unsigned int number_of_arcs)
{
    ...
    {
        if (number_of_arcs < 1)
            output() << "state has no arcs" << std::endl;
        else
...
bool
XfstCompiler::can_level_be_reached(int level, size_t whole_path_length)
{
    if (level == EOF || level == 0)
    {
        output() << "could not read level number (type '0' if you wish to "
                    "exit program)"
                 << std::endl;
        flush(&output());
        return false;
    ...
    {
        output() << "no such level: '" << level << "' (current level is "
                 << (int)whole_path_length << ")" << std::endl;
        flush(&output());
        return false;
    }
    return true;
...
XfstCompiler::inspect_net()
{
    ...
    while ((line = xfst_getline(stdin)) != NULL)
    {
        ...
        if (strcmp(line, "<\n") == 0 || strcmp(line, "<") == 0)
        {
            ...
            else if (!return_to_level(
                         whole_path, shortest_path,
                         hfst::size_t_to_uint(whole_path.size() - 1)))
            {
                error() << "FATAL ERROR: could not return to level '"
                        << (int)(whole_path.size() - 1) << "'" << std::endl;
                flush(&error());
                ignore_history_after_index(ind);
                free(line);
                PROMPT_AND_RETURN_THIS;
        ...
        else
        {
            ...
            else
            {
                ...
                output() << "  " << tr.get_input_symbol() << ":"
                         << tr.get_output_symbol() << " --> ";
                append_state_to_paths(whole_path, shortest_path,
                                      tr.get_target_state());
            }
        }
...
contains_regexps(hfst::xre::XreCompiler &xre_)
{
    ...
    HfstTransducer *well_formed
        = xre_.compile("TempNotBracketStar \"^[\" TempNotBracketStar  [ "
                       "\"^]\" TempNotBracketStar \"^[\"  TempNotBracketStar "
                       "]*  \"^]\" TempNotBracketStar ;");
...
// XRE
static HfstTransducer *
contains_regexp_markers_on_one_side(hfst::xre::XreCompiler &xre_,
                                    bool input_side)
{
    HfstTransducer *retval = NULL;
    if (input_side)
    {
        retval = xre_.compile("[?:?|0:?|?:0]* [\"^[\":? | \"^]\":? | \"^[\":0 "
                              "| \"^]\":0] [?:?|0:?|?:0]*");
    }
    else // output side
    {
...
static bool
is_well_formed_for_compile_replace(const HfstTransducer *t,
                                   hfst::xre::XreCompiler &xre_)
{
...
static std::string
to_literal_regexp(const hfst::StringPairVector &path, bool input_side)
{
    std::string pathstr("[");
...
/*  static HfstTransducer * to_literal_transducer(const hfst::StringPairVector
& path, hfst::xre::XreCompiler & xre_)
{
  std::string pathstr("[");
  for (hfst::StringPairVector::const_iterator it = path.begin(); it !=
path.end(); it++)
    {
  ...
  if (pathstr == "[];")
    pathstr = "[0];";
  // debug
  //std::cerr << "to_literal_transducer: compiling expression: " << pathstr <<
std::endl; char * p = strdup(pathstr.c_str()); HfstTransducer * retval =
xre_.compile(p);  // XRE free(p); return retval;
  }*/

static std::string
to_regexp(const hfst::StringPairVector &path, bool input_side, bool retokenize)
{
    std::string pathstr("[");
    for (const auto &it : path)
    {
...
XfstCompiler::compile_replace_net(Level level)
{
    ...
    try
    {
        HfstReplacementsMap replacement_map
            = fsm.find_replacements((level == UPPER_LEVEL));
        ...
        for (const auto &it : replacement_map)
        {
            ...
            for (const auto &rit : replacements)
            {
                ...
                std::string regexp
                    = to_regexp(rit.second, (level == UPPER_LEVEL),
                                (variables_["retokenize"] == "ON"));
                std::string literal_regexp
                    = to_literal_regexp(rit.second, (level != UPPER_LEVEL));

                std::string cross_product_regexp = "[ ";
                if (level == UPPER_LEVEL)
                {
                    cross_product_regexp.append(regexp)
                        .append(" ] .x. [ ")
                        .append(literal_regexp)
                        .append(" ]");
                }
                else
                {
                    cross_product_regexp.append(literal_regexp)
                        .append(" ] .x. [ ")
                        .append(regexp)
                        .append(" ]");
                }

                HfstTransducer *replacement
                    = xre_.compile(cross_product_regexp);
                if (replacement == NULL)
                {
                    error() << "Could not compile regular expression in "
                               "compile-replace: "
                            << cross_product_regexp << "." << std::endl;
                    flush(&error());
                    xfst_lesser_fail();
                    prompt();
                    return *this;
    ...
    // filter out regexps
    HfstTransducer *cr = contains_regexp_markers_on_one_side(
        xre_, (level == UPPER_LEVEL) /*input side*/);
    cr->optimize();
...
XfstCompiler::print_transducer_info()
{
    if (verbose_ && !stack_.empty())
    {
        ...
        output() << "? bytes. " << top->number_of_states() << " states, "
                 << top->number_of_arcs() << " arcs, ? paths" << std::endl;
        auto it = variables_.find("print-sigma");
        if (it != variables_.end() && it->second == "ON")
        {
            (const_cast<XfstCompiler *>(this))
                ->print_sigma(output_, false /* no prompt*/);
        }
    }
    flush(&output());
    return *this;
