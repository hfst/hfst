...
token_update_positions(const char *token)
{
    ...
    if (0 == newlines)
    {
        hlexclloc.last_column
            = (hlexclloc.first_column + hfst::size_t_to_int(token_length));
    }
    else
    {
...
{
    // N.B. reason for this error format is automagic support by
    // vim/emacs/jedit must be â€œfilename:lineno:colno-lineno:colno: stuffâ€�
    // c.f. http://www.gnu.org/prep/standards/standards.html#Errors
    char *filenames_lines_cols
        = (char *)malloc(sizeof(char) * (strlen(hlexcfilename) + 100));
    // linenumbers and columns
    if ((hlexclloc.first_line == hlexclloc.last_line)
        && (hlexclloc.first_column == (hlexclloc.last_column - 1)))
    {
        // TRANSLATORS: filename, line and column
        (void)sprintf(filenames_lines_cols, "%s:%d.%d", hlexcfilename,
    ...
    else if (hlexclloc.first_line == hlexclloc.last_line)
    {
        ...
        (void)sprintf(filenames_lines_cols, "%s:%d.%d-%d", hlexcfilename,
                      hlexclloc.first_line, hlexclloc.first_column,
                      hlexclloc.last_column);
    }
    else
    {
        ...
        (void)sprintf(filenames_lines_cols, "%s:%d.%d-%d.%d", hlexcfilename,
                      hlexclloc.first_line, hlexclloc.first_column,
...
strdup_token_part()
{
    ...
    {
        (void)sprintf(error_token, "[near: `%30s...' (truncated)]", hlexctext);
    }
    return error_token;
}
...
strip_percents(const char *s, bool do_zeros)
{
    ...
    if (do_zeros)
    {
        ...
        rv = (char *)malloc(sizeof(char) * strlen(s) * strlen("@0@") + 1);
    }
    else
    {
        // worst case scenario is string of %0's...
        rv = (char *)malloc(
            sizeof(char) * ((strlen(s) / 2) + 1) * strlen("@ZERO@") + 1);
    }
    char *p = rv;
    const char *c = s;
    bool escaping = false;
...
char *
strdup_nonconst_part(const char *token, const char *prefix, const char *suffix,
                     bool strip)
{
    size_t prefix_len = 0;
    size_t suffix_len = 0;
    ...
    {
        assert(strncmp(token + prefix_len + varpart_len, suffix, suffix_len)
               == 0);
    }
...
find_med_alingment(const vector<string> &s1, const vector<string> &s2)
{
    ...
    vector<vector<unsigned int> > dir(len1 + 1,
    ...
    for (unsigned int i = 1; i <= len1; ++i)
    {
        for (unsigned int j = 1; j <= len2; ++j)
        {
            ...
            // It's important to prioritize "del" when it has the same value as
            // "ins"" because we want the first string to have zeroes before
            // the second one. For example, if we have two strings: abc and
            // xyz, we would prefer the output 000abc:xyz000 over
            // abc000:000xyz, because we need to invert the transducer to use
            // it for the analysis and in lookup we want zeroes as late as
            // possible on the upper side. Anyway, the order of this two
            // following blocks determines the order of zeroes on upper/lower
            // side
    ...
    int x, y, i;
    for (x = hfst::size_t_to_int(s1.size()),
        y = hfst::size_t_to_int(s2.size()), i = 0;
         (x > 0) || (y > 0); i++)
    {
