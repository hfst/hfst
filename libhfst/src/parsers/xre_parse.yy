...
namespace hfst
{
  namespace xre
  {
    // number of characters read, used for scanning function
    // definition xre for argument symbols
    extern unsigned int cr;
    extern bool harmonize_;
    extern bool harmonize_flags_;
    extern bool allow_extra_text_at_end;

    // to control how many times a warning is given
    bool has_weight_been_zeroed = false;
...
%union {
   ...
   std::pair<hfst::xeroxRules::ReplaceArrow,
             std::vector<hfst::xeroxRules::Rule> >* replaceRuleVectorWithArrow;
   std::pair< hfst::xeroxRules::ReplaceArrow,
              hfst::xeroxRules::Rule>* replaceRuleWithArrow;
   std::pair< hfst::xeroxRules::ReplaceArrow,
              hfst::HfstTransducerPairVector>* mappingVectorWithArrow;
   std::pair< hfst::xeroxRules::ReplaceArrow,
              hfst::HfstTransducerPair>* mappingWithArrow;

   std::pair<hfst::xeroxRules::ReplaceType,
             hfst::HfstTransducerPairVector>* contextWithMark;
...
%left  CROSS_PRODUCT COMPOSITION LENIENT_COMPOSITION INTERSECTION
       MERGE_RIGHT_ARROW MERGE_LEFT_ARROW
...
REGEXP2: REPLACE {
        ...
        if ($1->has_flag_diacritics() && $3->has_flag_diacritics())
          {
            if (! harmonize_flags_)
            {
                 hfst::xre::warn("both composition arguments contain flag "
                                 "diacritics that are not harmonized\n");
            }
            else
            {
         ...
         catch (const FlagDiacriticsAreNotIdentitiesException & )
         {
            xreerror("flag diacritics must be identities in composition "
                     "if flag-is-epsilon is ON.\n"
                     "I.e. only FLAG:FLAG is allowed, not FLAG1:FLAG2, "
                     "FLAG:bar or foo:FLAG\n"
                     "Apply twosided flag-diacritics (tfd) "
                     "before composition.\n");
            YYABORT;
          }
          delete $3;
       ...
       | REGEXP2 MERGE_RIGHT_ARROW REPLACE {
          ...
          catch (const TransducersAreNotAutomataException & )
          {
            xreerror("transducers must be automata in merge operation.");
            delete $1;
            delete $3;
            YYABORT;
          }
       ...
       | SUB1 SUB2 SUB3 {
            ...
            {
                hfst::xre::warn("using definition as an ordinary label, "
                                "cannot substitute\n");
                $$ = & $1->optimize();
            }
            ...
            else
            {
                ...
                if (!empty_replace_transducer)
                {
                    // a:b .o. b -> x | y
                    // [[a:b].i .o. b -> x | y].i -
                    // this is for cases when b is on left side
                    // build Replace transducer
                    HfstTransducerPair mappingPair(HfstTransducer($2, $2,
                                                   hfst::xre::format), *$3);
                    ...
                    // if we are replacing with flag diacritics,
                    // the rule must allow flags to be replaced with themselves
...
SUB3: SYMBOL_LIST RIGHT_BRACKET {  $$ = $1;  }  // symbol list
      |
      RIGHT_BRACKET {
        $$ = new HfstTransducer(hfst::xre::format); // an empty symbol list
        }
      ;
       ...
       |  PARALLEL_RULES
         {
            ...
            case E_RTL_LONGEST_MATCH:
                $$ = new HfstTransducer(
                    replace_rightmost_longest_match( $1->second ) );
                break;
            case E_RTL_SHORTEST_MATCH:
                $$ = new HfstTransducer(
                    replace_rightmost_shortest_match($1->second) );
                break;
            case E_LTR_LONGEST_MATCH:
                $$ = new HfstTransducer(
                    replace_leftmost_longest_match( $1->second ) );
                break;
            case E_LTR_SHORTEST_MATCH:
                $$ = new HfstTransducer(
                    replace_leftmost_shortest_match( $1->second ) );
                break;
...
PARALLEL_RULES: PARALLEL_RULES COMMACOMMA RULE
         {
            ...
            $$ =  new std::pair< ReplaceArrow, std::vector<Rule> > ($3->first,
                                                                    $1->second);
            delete $1; delete $3;
         }
         | RULE
         {
            ...
            $$ =  new std::pair< ReplaceArrow, std::vector<Rule> > ($1->first,
                                                                   *ruleVector);
            delete ruleVector;
            delete $1;
         }
...
RULE: MAPPINGPAIR_VECTOR
      {
        ...
        // HfstTransducer allMappingsDisjuncted
        // = disjunctVectorMembers($1->second);
        Rule rule( $1->second );;
...
MAPPINGPAIR_VECTOR: MAPPINGPAIR_VECTOR COMMA MAPPINGPAIR
      {
        // std::cerr << "mapping_vector : mapping_vector comma mapping"
        //           << std::endl;
        // check if new Arrow is the same as the first one
        if ($1->first != $3->first)
        {
            hfst::xre::warn("Replace arrows should be the same. "
                            "Calculated as if all replacements "
                            "had the first arrow.");
        }
        $1->second.push_back($3->second);
        $$ =  new std::pair< ReplaceArrow, HfstTransducerPairVector>($1->first,
                                                                    $1->second);
        delete $1;
        delete $3;
      }
...
MAPPINGPAIR: REPLACE REPLACE_ARROW REPLACE
      {
        ...
        $$ =  new std::pair< ReplaceArrow, HfstTransducerPair> ($2,
                                                                mappingPair);
        delete $1;
        delete $3;
      }
      | REPLACE REPLACE_ARROW REPLACE MARKUP_MARKER REPLACE
      {
        ...
        HfstTransducerPair tmpMappingPair(*$1,
                                            HfstTransducer(hfst::xre::format));
       ...
       | LEFT_BRACKET_DOTTED RIGHT_BRACKET_DOTTED REPLACE_ARROW REPLACE
      {
        ...
        $$ =  new std::pair< ReplaceArrow, HfstTransducerPair> ($3,
                                                                mappingPair);
        delete $4;
      }
      | LEFT_BRACKET_DOTTED REPLACE RIGHT_BRACKET_DOTTED REPLACE_ARROW REPLACE
      {
        ...
        $$ =  new std::pair< ReplaceArrow, HfstTransducerPair> ($4,
                                                                mappingPair);
        delete $2;
        delete $5;
      }
...
CONTEXTS_VECTOR: CONTEXT
      {
        HfstTransducerPairVector * ContextVector = new
...
CONTEXT: REPLACE CENTER_MARKER REPLACE
      {
        if (hfst::xre::has_non_identity_pairs($1))
        {
            xreerror("Contexts need to be automata");
            YYABORT;
        }
        if (hfst::xre::has_non_identity_pairs($3))
        {
            xreerror("Contexts need to be automata");
            YYABORT;
        }
...
REGEXP5: REGEXP6 { $$ = $1; }
       ...
       | REGEXP5 INTERSECTION REGEXP6 {
            ...
            $$ = & $1->intersect(*$3, harmonize_).optimize()
...
REGEXP8: REGEXP9 { $$ = $1; }
       | COMPLEMENT REGEXP8 {
        ...
        if (! $2->is_automaton())
        {
            xreerror("Complement operator ~ is defined only for automata\n"
                     "Use expression [[?:?] - A]] instead "
                     "where A is the relation to be complemented.");
            YYABORT;
        }
            HfstTransducer complement =
                HfstTransducer::identity_pair( hfst::xre::format );
       ...
       | CONTAINMENT REGEXP8 {
            ...
            if (hfst::xre::has_non_identity_pairs($2))
            {
                hfst::xre::warn("using transducer that is not an automaton"
                              "in containment\n");
...
REGEXP11: REGEXP12 { $$ = $1; }
        ...
        | LEFT_BRACKET REGEXP2 RIGHT_BRACKET PAIR_SEPARATOR CURLY_BRACKETS {
            HfstTransducer * tmp =
                hfst::xre::xfst_curly_label_to_transducer($5,$5);
            free($5);
            $$ = & $2->cross_product(*tmp);
            delete tmp;
        }
        ...
        | LEFT_BRACKET REGEXP2 RIGHT_BRACKET WEIGHT {
            $$ = & $2->set_final_weights(hfst::double_to_float($4),
                                         true).optimize();
        }
        ...
        | READ_TEXT {
            ...
            else
            {
                ...
                HfstTransducer * retval =
                    new HfstTransducer(tmp, hfst::xre::format);
                retval->optimize();
                $$ = retval;
            }
        }
            ...
            else
            {
                ...
                while( fgets(line, 1000, f) != NULL )
                {
                    ...
                    StringPairVector spv =
                        HfstTokenizer::tokenize_space_separated(line);
                    tmp.disjunct(spv, 0);
                }
                fclose(f);
                HfstTransducer * retval =
        ...
        | READ_PROLOG {
            ...
            else
            {
                try
                {
                    ...
                    HfstBasicTransducer tmp =
                        HfstBasicTransducer::read_in_prolog_format(f,
                            linecount);
                    fclose(f);
...
LABEL: HALFARC {
     ...
     | FUNCTION REGEXP_LIST RIGHT_PARENTHESIS {
        ...
        else
        {
            ...
            xrelex_init(&scanner);
            YY_BUFFER_STATE bs =
                xre_scan_string(hfst::xre::get_function_xre($1),
                                scanner);
            // define special variables so that function arguments get
            // the values given in regexp list
            ...
            // do not include the characters read
            // when evaluating functions inside it
            unsigned int chars_read = hfst::xre::cr;
            int parse_retval = xreparse(scanner);
            hfst::xre::cr = chars_read;
