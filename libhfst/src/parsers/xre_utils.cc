...
void
set_substitution_function_symbol(const std::string &symbol)
{
    substitution_function_symbol = symbol;
}

bool
substitution_function(const hfst::StringPair &p, hfst::StringPairSet &sps)
{
...
parse_quoted(const char *s, unsigned int &length)
{
    ...
    while (*p != '\0')
    {
        ...
        else if (*p == '\\')
        {
            ...
            case '6':
            case '7':
                *err << "*** XRE unimplemented: parse octal escape in "
                     << std::string(p);
                xreflush(err);
                *r = '\0';
                p = p + 5;
            ...
            case 't':
                *r = '\t';
                ...
                break;
            case 'u':
                *err << "Unimplemented: parse unicode escapes in "
                     << std::string(p);
                xreflush(err);
                *r = '\0';
                r++;
            ...
            case 'x':
            {
                ...
                {
                    *err << "*** XRE unimplemented: parse \\x" << i
                         << std::endl;
                    xreflush(err);
    ...
    *r = '\0';
    ...
    length = hfst::HfstTokenizer::check_utf8_correctness_and_calculate_length(
        std::string(rv));

    return rv;
}

// If \a str is of form "@_<foo>_@", insert pair ("@_<foo>_@", "<foo>") into \a
// substitutions.
static void
insert_angle_bracket_substitutions(
    const std::string &str, hfst::HfstSymbolSubstitutions &substitutions)
{
    if (str.length() < 6)
        return;
    if (str.substr(0, 3) == "@_<" && str.substr(str.length() - 3, 3) == ">_@")
    {
...
escape_enclosing_angle_brackets(char *s)
{
    ...
    std::string retval
        = std::string("@_") + std::string(s) + std::string("_@");
    free(s);
...
compile(const string &xre, map<string, HfstTransducer *> &defs,
        map<string, string> &func_defs, map<string, unsigned int> func_args,
        map<string, std::set<string> > &lists, ImplementationType impl)
{
    // lock here?
...
// todo: Contains lots of code copied directly from compile(...), should be
// rewritten..
HfstTransducer *
...
bool
is_valid_function_call(const char *name,
                       const std::vector<HfstTransducer> *args)
{
    ...
    if (number_of_args != args->size())
    {
        ...
        *err << "Wrong number of arguments: function '" << name << "' expects "
             << (int)number_of_args << ", " << (int)args->size() << " given"
             << std::endl;
        xreflush(err);
        // fprintf(stderr, "Wrong number of arguments: function '%s' expects
        // %i, %i given\n",
...
const char *
get_function_xre(const char *name)
{
...
bool
define_function_args(const char *name, const std::vector<HfstTransducer> *args)
{
         ...
         it != args->end(); it++)
    {
        ...
        // arg_number); // DEBUG std::cerr << *it << std::endl;
        arg_number++;
    }
    return true;
}
...
void
undefine_function_args(const char *name)
{
...
bool
is_definition(const char *symbol)
{
    ...
    if (expand_definitions)
    {
             ...
             it != definitions.end(); it++)
        {
            if (strcmp(it->first.c_str(), symbol) == 0)
            {
                ...
                // if (it->second->get_alphabet().find(symbol) !=
                // it->second->get_alphabet().end())
                //   std::cerr << "WARN: symbol '" << std::string(symbol) << "'
...
xfst_curly_label_to_transducer(const char *input, const char *output)
{
    ...
    if (strcmp(input, hfst::internal_unknown.c_str()) == 0)
    {
        ...
        for (StringVector::const_iterator it = sv.begin(); it != sv.end();
             it++)
        {
...
xfst_label_to_transducer(const char *input, const char *output)
{
    ...
    bool input_is_unknown
        = (strcmp(input, hfst::internal_unknown.c_str()) == 0);
    bool output_is_unknown
    ...
    {
        HfstTransducer *tmp
            = NULL; // temporary transducer for cross-product calculation
        if (input_is_unknown)
        {
            retval = new HfstTransducer(hfst::internal_identity,
                                        hfst::xre::format);
    ...
    {
        retval = new HfstTransducer(hfst::internal_unknown,
                                    hfst::internal_unknown, hfst::xre::format);
        HfstTransducer id(hfst::internal_identity, hfst::internal_identity,
                          hfst::xre::format);
        retval->disjunct(id).minimize(); // it should be safe to minimize
    }
    else if (input_is_unknown)
    {
        retval = new HfstTransducer(hfst::internal_unknown, output,
                                    hfst::xre::format);
        HfstTransducer output_tr(output, output, hfst::xre::format);
        retval->disjunct(output_tr)
            .minimize(); // it should be safe to minimize
    }
    else if (output_is_unknown)
    {
        retval = new HfstTransducer(input, hfst::internal_unknown,
                                    hfst::xre::format);
...
HfstTransducer *
contains(const HfstTransducer *t)
{
...
// [ 0::weight -> 0 || _ [t] ] - [?* - $[t]]
HfstTransducer *
contains_with_weight(const HfstTransducer *t, float weight)
{
    ...
    hfst::xeroxRules::Rule rule(mappingPairVector, contextPairVector,
                                hfst::xeroxRules::REPL_UP);
    HfstTransducer weighted_rule(hfst::xre::format);
    weighted_rule = replace(rule, false);

    // note that replace_leftmost_longest_match does not work correctly in the
    // case of epsilons
...
// [$c - $[ [[?+ c ?*] & [c ?*]] | [[c ?+] & [c]]]]
HfstTransducer *
contains_once(const HfstTransducer *c)
{
    // any_star = [?*]
...
HfstTransducer *
contains_once_optional(const HfstTransducer *t)
{
    // neg_t = ~$[t]
...
HfstTransducer *
merge_first_to_second(HfstTransducer *tr1, HfstTransducer *tr2)
{
    // Merge operation creates an XreCompiler that needs this information
    // below. Otherwise, it will overwrite all this.
    struct XreConstructorArguments args(
        hfst::xre::definitions, hfst::xre::function_definitions,
        hfst::xre::function_arguments, hfst::xre::symbol_lists,
        hfst::xre::format);
...
void
warn_about_xfst_special_symbol(const char *symbol)
{
    ...
    if (!verbose_)
        return;
    std::ostream *err = xreerrstr();
    *err << "warning: '" << symbol << " ' is an ordinary symbol in hfst"
         << std::endl;
    xreflush(err);
}

void
warn_about_hfst_special_symbol(const char *symbol)
{
    if (symbol[0] != '@')
        return;
    if (symbol[1] != '_')
    ...
    {
        std::ostream *err = xreerrstr();
        *err << "warning: '" << symbol << "' is not an ordinary symbol in hfst"
             << std::endl;
        xreflush(err);
    }
}
...
void
warn_about_special_symbols_in_replace(HfstTransducer *t)
{
    if (!verbose_)
        return;
         ...
         it++)
    {
        ...
        {
            *err << "warning: using special symbol '" << *it
                 << "' in replace rule, use substitute instead" << std::endl;
        }
    }
    xreflush(err);
}
...
void
check_multichar_symbol(const char *symbol)
{
...
bool
has_non_identity_pairs(const HfstTransducer *t)
{
